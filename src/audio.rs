...
mod tests {
    ...
    #[test]
    fn ui_action_sfx_table_matches_bevy() {
        // MainMenuPlay: Click + MenuCharSelect, both 0.7.
        let cues = ui_action_sfx(&UiAction::MainMenuPlay);
        assert_eq!(cues.len(), 2);
        assert_eq!(cues[0].name, "sndClick");
        assert_eq!(cues[1].name, "sndMenuCharSelect");
        assert!(cues.iter().all(|c| (c.volume - 0.7).abs() < 1e-6));
        // Quits back out; silent actions stay silent.
        let quit = ui_action_sfx(&UiAction::QuitApp);
        assert_eq!((quit[0].name, quit[0].volume), ("sndClickBack", 0.6));
        assert!(ui_action_sfx(&UiAction::StartGame).is_empty());
        assert!(ui_action_sfx(&UiAction::Resume).is_empty());
        assert!(ui_action_sfx(&UiAction::OpenSettings).is_empty());
        // Settings commits click; sliders let go.
        let toggle = ui_action_sfx(&UiAction::SettingToggle("x".to_string()));
        assert_eq!((toggle[0].name, toggle[0].volume), ("sndClick", 0.6));
        let slider = ui_action_sfx(&UiAction::SettingSlider {
            key: "x".to_string(),
            value: 1.0,
        });
        assert_eq!((slider[0].name, slider[0].volume), ("sndSliderLetGo", 0.5));
        let credits = ui_action_sfx(&UiAction::SettingViewCredits);
        assert_eq!((credits[0].name, credits[0].volume), ("sndMenuCredits", 0.7));
    }

    #[test]
    fn race_skin_crown_denial_stings() {
        // GML Slct law: Fish → Mutant1, Skeleton → BloodGamble.
        assert_eq!(race_select_sfx(crate::data::RaceId::Fish).name, "sndMutant1Slct");
        assert_eq!(
            race_select_sfx(crate::data::RaceId::Skeleton).name,
            "sndBloodGamble"
        );
        assert_eq!(skin_select_sfx(0).name, "sndMenuASkin");
        assert_eq!(skin_select_sfx(1).name, "sndMenuBSkin");
        assert_eq!(skin_select_sfx(2).name, "sndMenuCSkin");
        assert_eq!(denied_sfx().name, "sndNoSelect");
        assert_eq!(hover_sfx().name, "sndHover");
        assert_eq!(crown_select_sfx().name, "sndMenuCrown");
    }

    #[test]
    fn hurt_cue_queues() {
        let audio = GameAudio;
        let mut q: Queue<AudioCue> = Queue::default();
        audio.play_hurt(&mut q);
        assert_eq!(
            q.drain(),
            vec![AudioCue {
                name: "sndPlayerHit",
    ...
    #[test]
    fn sfx_stem_table_volumes() {
        let audio = GameAudio;
        let mut q: Queue<AudioCue> = Queue::default();
        audio.play_shoot(&mut q);
        audio.play_empty(&mut q);
        audio.play_van_warning(&mut q);
        audio.play_chest(&mut q);
        assert_eq!(
            q.drain(),
            vec![
                AudioCue {
    ...
    #[test]
    fn resolve_sfx_falls_back_to_audio_wav() {
        assert_eq!(resolve_sfx_path("sndPistol"), "audio/sndPistol.wav");
    }

    #[test]
    fn weapon_fire_dispatch_branches() {
        // Underwater override wins over everything.
        assert_eq!(
            GameAudio::weapon_fire_cue("GOLD PLASMA GUN", true),
            ("sndOasisShoot", 0.5, 0.1)
        );
        // Gold tiers.
        assert_eq!(
            GameAudio::weapon_fire_cue("GOLDEN REVOLVER", false),
            ("sndGoldPistol", 0.5, 0.1)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("GOLD MACHINEGUN", false),
            ("sndGoldMachinegun", 0.5, 0.1)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("GOLD SHOTGUN", false),
            ("sndGoldShotgun", 0.5, 0.1)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("GOLD CROSSBOW", false),
            ("sndGoldCrossbow", 0.5, 0.1)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("GOLD GRENADE LAUNCHER", false),
            ("sndGoldGrenade", 0.5, 0.1)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("GOLD PLASMA GUN", false),
            ("sndGoldPlasma", 0.5, 0.1)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("GOLD LASER", false),
            ("sndGoldLaser", 0.5, 0.1)
        );
        // Energy families.
        assert_eq!(
            GameAudio::weapon_fire_cue("PLASMA GUN", false),
            ("sndPlasma", 0.55, 0.08)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("GUN GUN", false),
            ("sndPlasma", 0.55, 0.08)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("LASER RIFLE", false),
            ("sndLaser", 0.55, 0.08)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("LIGHTNING PISTOL", false),
            ("sndLightningPistol", 0.55, 0.08)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("FLAME CANNON", false),
            ("sndFlameCannon", 0.55, 0.08)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("DISC GUN", false),
            ("sndDiscgun", 0.55, 0.08)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("SLUGGER", false),
            ("sndSlugger", 0.6, 0.08)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("SPLINTER GUN", false),
            ("sndSplinterGun", 0.5, 0.08)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("GRENADE LAUNCHER", false),
            ("sndGrenade", 0.6, 0.08)
        );
        // Slug tiers.
        assert_eq!(
            GameAudio::weapon_fire_cue("HEAVY CROSSBOW", false),
            ("sndCrossbow", 0.5, 0.08)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("SHOTGUN", false),
            ("sndShotgun", 0.6, 0.1)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("MACHINEGUN", false),
            ("sndMachinegun", 0.4, 0.15)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("REVOLVER", false),
            ("sndPistol", 0.5, 0.12)
        );
        assert_eq!(
            GameAudio::weapon_fire_cue("SENTRY GUN", false),
            ("sndMachinegun", 0.4, 0.15)
        );
        // Unknown falls back to the pop-gun stem.
        assert_eq!(
            GameAudio::weapon_fire_cue("WEIRD WAND", false),
            ("sndPistol", 0.5, 0.12)
        );

        let audio = GameAudio;
        let mut q: Queue<AudioCue> = Queue::default();
        audio.play_weapon_fire_gml(&mut q, "PLASMA GUN", false);
        audio.play_weapon_fire(&mut q, "SHOTGUN");
        assert_eq!(
            q.drain(),
            vec![
                AudioCue {
    ...
    #[test]
    fn music_and_ambience_per_area() {
        use AreaId::*;
        let rows: [(AreaId, MusicCue, AmbienceCue); 17] = [
            (Desert, MusicCue::Desert, AmbienceCue::DesertWind),
            (Sewers, MusicCue::Sewers, AmbienceCue::SewerDrip),
            (PizzaSewers, MusicCue::PizzaSewers, AmbienceCue::SewerDrip),
            (
                Scrapyards,
                MusicCue::Scrapyards,
                AmbienceCue::ScrapHum,
            ...
            (City, MusicCue::City, AmbienceCue::CityNoise),
            (Jungle, MusicCue::Jungle, AmbienceCue::JungleBugs),
            (Labs, MusicCue::Labs, AmbienceCue::LabBuzz),
            (Palace, MusicCue::Palace, AmbienceCue::PalaceFire),
            (HQ, MusicCue::Hq, AmbienceCue::HqSirens),
            (Oasis, MusicCue::Oasis, AmbienceCue::OasisBreeze),
            (Vault, MusicCue::Vault, AmbienceCue::VaultHum),
            (
                CrownVault,
                MusicCue::CrownVault,
                AmbienceCue::VaultHum,
            ...
            (Loop, MusicCue::Desert, AmbienceCue::DesertWind),
        ];
        for (area, music, ambience) in rows {
            assert_eq!(music_for_area(area), music, "{area:?}");
            assert_eq!(ambience_for_area(area), ambience, "{area:?}");
            assert!(!music_candidates(music).is_empty());
            assert_eq!(music_path(music), Some(music_candidates(music)[0]));
            if ambience != AmbienceCue::None {
                assert!(!ambience_candidates(ambience).is_empty());
            }
        }
        assert!(ambience_candidates(AmbienceCue::None).is_empty());
        assert_eq!(ambience_path(AmbienceCue::None), None);
    }

    #[test]
    fn boss_music_override() {
        assert_eq!(
            boss_music_for_kind(EnemyKind::BigBandit),
            Some(MusicCue::BossBigBandit)
        );
        assert_eq!(
            boss_music_for_kind(EnemyKind::BigBanditLoop),
            Some(MusicCue::BossBigBandit)
        );
        assert_eq!(
            boss_music_for_kind(EnemyKind::BigDog),
            Some(MusicCue::BossBigDog)
        );
        assert_eq!(
            boss_music_for_kind(EnemyKind::BigDogLoop),
            Some(MusicCue::BossBigDog)
        );
        assert_eq!(
            boss_music_for_kind(EnemyKind::LilHunter),
            Some(MusicCue::BossLilHunter)
        );
        assert_eq!(
            boss_music_for_kind(EnemyKind::LilHunterLoop),
            Some(MusicCue::BossLilHunter)
        );
        assert_eq!(
            boss_music_for_kind(EnemyKind::Throne),
            Some(MusicCue::BossThrone)
        );
        assert_eq!(
            boss_music_for_kind(EnemyKind::ThroneII),
            Some(MusicCue::BossThroneII)
        );
        assert_eq!(boss_music_for_kind(EnemyKind::Bandit), None);
        assert_eq!(boss_music_for_kind(EnemyKind::Maggot), None);

        // Boss beats area, campfire beats boss.
        assert_eq!(
            desired_music_cue(AreaId::Desert, false, [EnemyKind::BigBandit]),
            MusicCue::BossBigBandit
        );
        assert_eq!(
            desired_music_cue(AreaId::Palace, false, [EnemyKind::ThroneII]),
            MusicCue::BossThroneII
        );
        assert_eq!(
            desired_music_cue(AreaId::Desert, true, [EnemyKind::BigBandit]),
            MusicCue::Campfire
        );
        assert_eq!(
            desired_music_cue(AreaId::Desert, false, [EnemyKind::Bandit]),
            MusicCue::Desert
        );
        assert_eq!(
            desired_ambience_cue(AreaId::Desert, true),
            AmbienceCue::CampfireCrackle
        );
        assert_eq!(
            desired_ambience_cue(AreaId::Sewers, false),
            AmbienceCue::SewerDrip
        );
    }

    fn area_world(app: AppState, area: AreaId, game_over: bool) -> World {
    ...
    #[test]
    fn sync_area_audio_state_gating() {
        // Splash silences everything.
        let mut world = area_world(AppState::Splash, AreaId::Desert, false);
        let mut schedule = Schedule::default();
        schedule.add_systems(sync_area_audio);
        schedule.run(&mut world);
        let state = world.resource::<AreaAudioState>();
        assert_eq!((state.current_music, state.current_ambience), (None, None));

        // Menus hold the title theme.
        for app in [AppState::MainMenu, AppState::Title] {
            let mut world = area_world(app, AreaId::Desert, false);
            let mut schedule = Schedule::default();
            schedule.add_systems(sync_area_audio);
            schedule.run(&mut world);
            let state = world.resource::<AreaAudioState>();
            assert_eq!(
                (state.current_music, state.current_ambience),
                (Some(MusicCue::TitleTheme), None),
                "{app:?}"
            );
        }

        // Loading previews the area; game-over silences InGame.
        let mut world = area_world(AppState::Loading, AreaId::Sewers, false);
        let mut schedule = Schedule::default();
        schedule.add_systems(sync_area_audio);
        schedule.run(&mut world);
        let state = world.resource::<AreaAudioState>();
        assert_eq!(
            (state.current_music, state.current_ambience),
            (Some(MusicCue::Sewers), Some(AmbienceCue::SewerDrip))
        );

        let mut world = area_world(AppState::InGame, AreaId::Sewers, true);
        let mut schedule = Schedule::default();
        schedule.add_systems(sync_area_audio);
        schedule.run(&mut world);
        let state = world.resource::<AreaAudioState>();
        ...
        // InGame follows the area track.
        let mut world = area_world(AppState::InGame, AreaId::Palace, false);
        let mut schedule = Schedule::default();
        schedule.add_systems(sync_area_audio);
        schedule.run(&mut world);
        let state = world.resource::<AreaAudioState>();
        assert_eq!(
            (state.current_music, state.current_ambience),
            (Some(MusicCue::Palace), Some(AmbienceCue::PalaceFire))
        );

        // A live boss overrides the area track.
        let mut world = area_world(AppState::InGame, AreaId::FrozenCity, false);
        world.spawn((
            Enemy {
                kind: EnemyKind::BigDog,
                score: 0,
                touch_damage: 0,
                rad_drop: 0,
                drop_chance: 0,
            ...
            BossBrain::new(EnemyKind::BigDog, glam::Vec2::ZERO),
        ));
        let mut schedule = Schedule::default();
        schedule.add_systems(sync_area_audio);
        schedule.run(&mut world);
        assert_eq!(
            world.resource::<AreaAudioState>().current_music,
            Some(MusicCue::BossBigDog)
        );

        // Campfire rest overrides the boss.
        world.spawn(CampfireProp);
        schedule.run(&mut world);
        let state = world.resource::<AreaAudioState>();
        assert_eq!(
            (state.current_music, state.current_ambience),
            (
                ...
                Some(AmbienceCue::CampfireCrackle)
            )
        );
    }
    ...
    fn mix_world() -> World {
        ...
        world.init_resource::<AudioChannels>();
    ...
    #[test]
    fn area_fades_converge_and_remix() {
        let mut world = area_world(AppState::InGame, AreaId::Desert, false);
        world.insert_resource(Paused(false));
        let mut sync = Schedule::default();
        sync.add_systems(sync_area_audio);
        sync.run(&mut world);

        // Move the mix resources over and tick past the 1.2 s music fade.
        let mut world2 = mix_world();
        let state = world.resource::<AreaAudioState>().music_fade;
        world2.resource_mut::<AreaAudioState>().music_fade = state;
        world2.resource_mut::<AreaAudioState>().current_music = Some(MusicCue::Desert);
        world2.resource_mut::<AreaAudioState>().current_ambience =
            Some(AmbienceCue::DesertWind);
        world2.resource_mut::<AreaAudioState>().ambience_fade = AreaAudioFader {
            start: 0.0,
            end: 1.0,
            timer: GTimer::from_seconds(AMBIENCE_FADE_SECS, TimerMode::Once),
        };
        let mut tick = Schedule::default();
        tick.add_systems(tick_area_audio_fades);
        for _ in 0..40 {
            tick.run(&mut world2);
        }
        let state = world2.resource::<AreaAudioState>();
        assert!((state.music_gain - 1.0).abs() < 1e-4);
        assert!((state.ambience_gain - 1.0).abs() < 1e-4);
        // master 1 * music 0.8 * dim 1.
        assert!((state.music_volume - 0.8).abs() < 1e-4);
        // master 1 * music 0.8 * 0.55 scale * filter 1.
        assert!((state.ambience_volume - 0.44).abs() < 1e-4);

        // Pause dims music only.
        world2.resource_mut::<Paused>().0 = true;
        tick.run(&mut world2);
        let state = world2.resource::<AreaAudioState>();
        assert!((state.music_volume - 0.8 * MUSIC_PAUSE_DIM).abs() < 1e-4);
        assert!((state.ambience_volume - 0.44).abs() < 1e-4);

        // Ducking the filter scales ambience only.
        world2.resource_mut::<AmbFilter>().0 = 0.2;
        tick.run(&mut world2);
        let state = world2.resource::<AreaAudioState>();
        assert!((state.ambience_volume - 0.44 * 0.2).abs() < 1e-4);
    }

    #[test]
    fn amb_filter_ducks_and_recovers() {
        let mut filter = AmbFilter::default();
        assert_eq!(amb_filter_target(false, false), 1.0);
        assert_eq!(amb_filter_target(true, false), 0.2);
        assert_eq!(amb_filter_target(false, true), 0.2);
        step_amb_filter(&mut filter, 0.2, 1.0 / 30.0);
        assert!((filter.0 - (1.0 - 3.0 / 30.0)).abs() < 1e-6);
        for _ in 0..60 {
            step_amb_filter(&mut filter, 1.0, 1.0 / 30.0);
        }
        assert_eq!(filter.0, 1.0);
    }

    #[test]
    fn throttle_allows_timing_law() {
        assert!(throttle_allows(ReactiveCue::Kill, None, 100.0));
        // Kill window is 0.12 s.
        assert!(!throttle_allows(
            ReactiveCue::Kill,
            Some(1.0),
            1.0 + 0.12 - 1e-4
        ));
        assert!(throttle_allows(ReactiveCue::Kill, Some(1.0), 1.0 + 0.12));
        // UiCycle window is 0.06 s (above the 1/30 floor).
        assert!(!throttle_allows(
            ReactiveCue::UiCycle,
            Some(0.0),
            0.059
        ));
        assert!(throttle_allows(ReactiveCue::UiCycle, Some(0.0), 0.06));
        // Default window is 0.38 s; long windows hold stale stamps shut.
        assert!(!throttle_allows(
            ReactiveCue::BossAppear,
            Some(0.0),
            0.379
        ));
        assert!(throttle_allows(ReactiveCue::BossAppear, Some(0.0), 0.38));
        assert!(!throttle_allows(
            ReactiveCue::KillStreak,
            Some(5.0),
            7.499
        ));
        assert!(throttle_allows(ReactiveCue::KillStreak, Some(5.0), 7.5));
        assert_eq!(cue_throttle_seconds(ReactiveCue::PlayerCritical), 1.5);
        assert_eq!(
            cue_throttle_seconds(ReactiveCue::WeaponPickup),
            cue_throttle_seconds(ReactiveCue::ChestOpen)
        );
    }

    #[test]
    fn cue_volume_bands() {
        assert_eq!(cue_base_volume(ReactiveCue::PlayerDeath), 1.0);
        assert_eq!(cue_base_volume(ReactiveCue::BossAppear), 0.95);
        assert_eq!(cue_base_volume(ReactiveCue::LoopComplete), 0.95);
        assert_eq!(cue_base_volume(ReactiveCue::LevelUp), 0.85);
        assert_eq!(cue_base_volume(ReactiveCue::PortalOpen), 0.75);
        assert_eq!(cue_base_volume(ReactiveCue::KillStreak), 0.70);
        assert_eq!(cue_base_volume(ReactiveCue::UiConfirm), 0.58);
        assert_eq!(cue_base_volume(ReactiveCue::UiClick), 0.48);
        assert_eq!(cue_base_volume(ReactiveCue::UiCycle), 0.40);
        assert_eq!(cue_base_volume(ReactiveCue::Kill), 0.32);
    }
    ...
    fn reactive_world() -> World {
        ...
        world.insert_resource(time);
        world.init_resource::<AudioChannels>();
        world.init_resource::<ReactiveAudioState>();
        world.init_resource::<Queue<ReactiveAudioRequest>>();
        world.init_resource::<Queue<ResolvedReactiveCue>>();
        world
    }

    #[test]
    fn flush_drains_queue_in_order() {
        let mut world = reactive_world();
        world.spawn(QueuedReactiveCue(ReactiveCue::Kill));
        world.spawn(QueuedReactiveCue(ReactiveCue::LevelUp));
        world.spawn(QueuedReactiveCue(ReactiveCue::BossAppear));
        let mut schedule = Schedule::default();
        schedule.add_systems(flush_queued_cues);
        schedule.run(&mut world);
        let got = world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .drain();
        assert_eq!(
            got.iter().map(|r| r.cue).collect::<Vec<_>>(),
            vec![
                ReactiveCue::Kill,
                ReactiveCue::LevelUp,
                ReactiveCue::BossAppear
        ...
        // Entities are consumed.
        let mut q = world.query::<&QueuedReactiveCue>();
        assert_eq!(q.iter(&world).count(), 0);
    }

    #[test]
    fn reactive_requests_resolve_throttle_and_bus() {
        let mut world = reactive_world();
        world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .push(ReactiveAudioRequest::new(ReactiveCue::Kill));
        let mut schedule = Schedule::default();
        schedule.add_systems(play_reactive_audio_requests);
        schedule.run(&mut world);
        let got = world
            .resource_mut::<Queue<ResolvedReactiveCue>>()
            .drain();
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].cue, ReactiveCue::Kill);
        assert_eq!(got[0].path, cue_candidates(ReactiveCue::Kill)[0]);
        // Kill base 0.32 * default bus 1.
        assert!((got[0].volume - 0.32).abs() < 1e-6);

        // Same tick: throttled away (stale `now` since time didn't move).
        world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .push(ReactiveAudioRequest::new(ReactiveCue::Kill));
        schedule.run(&mut world);
        ...
        // Muted bus clears the inbox without emitting.
        world.resource_mut::<AudioChannels>().master = 0.0;
        world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .push(ReactiveAudioRequest::new(ReactiveCue::BossAppear));
        schedule.run(&mut world);
        assert!(
            world
                .resource_mut::<Queue<ResolvedReactiveCue>>()
        ...
        assert!(
            world
                .resource::<Queue<ReactiveAudioRequest>>()
                .is_empty()
        );
    }

    #[test]
    fn observe_player_level_and_critical() {
        let mut world = reactive_world();
        world.spawn((
            Player::default(),
            Health {
        ...
        let mut schedule = Schedule::default();
        schedule.add_systems(observe_player_audio_state);
        schedule.run(&mut world);
        assert!(
            world
                .resource_mut::<Queue<ReactiveAudioRequest>>()
        ...
        // Level-up fires once on the rising edge.
        let mut player = world.query::<&mut Player>();
        player.single_mut(&mut world).unwrap().level = 2;
        schedule.run(&mut world);
        let got = world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .drain();
        assert_eq!(got.iter().map(|r| r.cue).collect::<Vec<_>>(), [ReactiveCue::LevelUp]);
        schedule.run(&mut world);
        assert!(
            world
                .resource_mut::<Queue<ReactiveAudioRequest>>()
        ...
        // Dropping to <= 10% of max (ceil(0.8) = 1) stings once per fall.
        let mut health = world.query::<&mut Health>();
        health.single_mut(&mut world).unwrap().hp = 1;
        schedule.run(&mut world);
        let got = world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .drain();
        assert!(got.iter().any(|r| r.cue == ReactiveCue::PlayerCritical));
    }

    #[test]
    fn observe_boss_appear_and_defeat() {
        let mut world = reactive_world();
        let mut schedule = Schedule::default();
        schedule.add_systems(observe_boss_audio_state);
        schedule.run(&mut world);
        assert!(
            world
                .resource_mut::<Queue<ReactiveAudioRequest>>()
        ...
        let got = world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .drain();
        assert_eq!(got.iter().map(|r| r.cue).collect::<Vec<_>>(), [ReactiveCue::BossAppear]);
    ...
    #[test]
    fn observe_kill_queues_and_streak_law() {
        // Streak law: every 10th kill inside the 3.5 s window streaks.
        let mut state = ReactiveAudioState::default();
        for i in 1..=20u32 {
            assert_eq!(state.note_kill(i as f32 * 0.1), i % 10 == 0);
        }
        // A gap resets the streak.
        assert!(!state.note_kill(20.0 * 0.1 + 3.6));
        assert!(!state.note_kill(20.0 * 0.1 + 3.7));

        // System wiring: each removed Enemy queues one Kill.
        let mut world = reactive_world();
        let mut schedule = Schedule::default();
        schedule.add_systems(observe_kill_audio_state);
        let e = world
            .spawn(Enemy {
        ...
        let got = world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .drain();
        assert_eq!(got.iter().map(|r| r.cue).collect::<Vec<_>>(), [ReactiveCue::Kill]);
    }

    #[test]
    fn reset_reactive_state_clears() {
        let mut world = reactive_world();
        world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .push(ReactiveAudioRequest::new(ReactiveCue::Kill));
        let mut play = Schedule::default();
        play.add_systems(play_reactive_audio_requests);
        play.run(&mut world);
        assert!(
            !world
                .resource_mut::<Queue<ResolvedReactiveCue>>()
        ...
        let mut reset = Schedule::default();
        reset.add_systems(reset_reactive_audio_state);
        reset.run(&mut world);
        let state = world.resource::<ReactiveAudioState>();
        assert!(state.last_fired.is_empty());
        assert!(state.low_hp_armed);
    }

    #[test]
    fn ui_action_mapping_coverage() {
        let rows: [(UiAction, Option<ReactiveCue>); 37] = [
            (UiAction::StartGame, Some(ReactiveCue::UiConfirm)),
            (UiAction::MainMenuPlay, Some(ReactiveCue::UiConfirm)),
            (UiAction::Resume, Some(ReactiveCue::UiConfirm)),
            (UiAction::QuitToTitle, Some(ReactiveCue::UiBack)),
            (UiAction::QuitApp, Some(ReactiveCue::UiBack)),
            (UiAction::ToggleLoadout, Some(ReactiveCue::UiBack)),
            (UiAction::OpenSettings, Some(ReactiveCue::UiClick)),
            (UiAction::OpenCredits, Some(ReactiveCue::UiClick)),
            (UiAction::CloseOverlay, Some(ReactiveCue::UiClick)),
            (UiAction::SaveSettings, Some(ReactiveCue::UiClick)),
            (
                UiAction::SelectCharacter(0),
                Some(ReactiveCue::UiConfirm),
            ),
            (UiAction::PickMutation(0), Some(ReactiveCue::UiConfirm)),
            (
                UiAction::SelectMutation(0),
                Some(ReactiveCue::UiConfirm),
            ),
            (UiAction::SelectCrown(0), Some(ReactiveCue::UiConfirm)),
            (UiAction::SelectSkin(0), Some(ReactiveCue::UiCycle)),
            (UiAction::NextLanguage, Some(ReactiveCue::UiCycle)),
            (
                UiAction::CycleStartWeapon(1),
                Some(ReactiveCue::UiCycle),
            ),
            (
                UiAction::CycleStoredWeapon(1),
                Some(ReactiveCue::UiCycle),
            ),
            (UiAction::CycleCrown(1), Some(ReactiveCue::UiCycle)),
            (UiAction::SetMasterVol(0.5), None),
            (UiAction::SetSfxVol(0.5), None),
            (UiAction::SetMusicVol(0.5), None),
            (UiAction::SetAmbienceVol(0.5), None),
            (UiAction::SetLanguage("en".into()), None),
            (
                UiAction::SettingsCategory(0),
                Some(ReactiveCue::UiClick),
            ),
            (UiAction::SettingsBack, Some(ReactiveCue::UiClick)),
            (
                UiAction::ShowPauseConfirm(0),
                Some(ReactiveCue::UiClick),
            ),
            (
                UiAction::CancelPauseConfirm,
            ...
            (UiAction::ConfirmPause(0), Some(ReactiveCue::UiClick)),
            (
                UiAction::SettingToggle("x".into()),
                Some(ReactiveCue::UiClick),
            ),
            (
                UiAction::SettingCycle {
            ...
            (
                UiAction::SettingOpenSubcategory(0),
                Some(ReactiveCue::UiClick),
            ),
            (
                UiAction::SettingSlider {
        ...
        assert_eq!(rows.len(), 37, "every UiAction variant covered");
        for (action, expected) in &rows {
            assert_eq!(ui_action_to_cue(action), *expected, "{action:?}");
        }

        // Bridging: mapped actions queue, silent ones don't.
        let mut world = reactive_world();
        world.init_resource::<Queue<UiBridgeAction>>();
        world
            .resource_mut::<Queue<UiBridgeAction>>()
            .push(UiBridgeAction(UiAction::Resume));
        world
            .resource_mut::<Queue<UiBridgeAction>>()
            .push(UiBridgeAction(UiAction::SetSfxVol(0.1)));
        world
            .resource_mut::<Queue<UiBridgeAction>>()
            .push(UiBridgeAction(UiAction::NextLanguage));
        let mut schedule = Schedule::default();
        schedule.add_systems(play_ui_action_audio);
        schedule.run(&mut world);
        let got = world
            .resource_mut::<Queue<ReactiveAudioRequest>>()
            .drain();
        assert_eq!(
            got.iter().map(|r| r.cue).collect::<Vec<_>>(),
            vec![ReactiveCue::UiConfirm, ReactiveCue::UiCycle]
        );
    }

    #[test]
    fn intensity_score_bands_and_target() {
        assert!(is_idpd_kind(EnemyKind::IdpdGrunt));
        assert!(is_idpd_kind(EnemyKind::IdpdShield));
        assert!(is_idpd_kind(EnemyKind::IdpdElite));
        assert!(is_idpd_kind(EnemyKind::IdpdVan));
        assert!(!is_idpd_kind(EnemyKind::IdpdInspector));
        assert!(!is_idpd_kind(EnemyKind::Bandit));

        // ordinary + 2*idpd + 4*boss.
        assert_eq!(combat_intensity_score(0, 0, 0), 0);
        assert_eq!(combat_intensity_score(3, 0, 0), 3);
        assert_eq!(combat_intensity_score(1, 1, 0), 2);
        assert_eq!(combat_intensity_score(0, 0, 1), 4);
        assert_eq!(combat_intensity_score(2, 1, 1), 6);

        // Bands: silent through 2, ramp 3..=9, full past 9.
        assert_eq!(combat_intensity_target(0), 0.0);
        assert_eq!(combat_intensity_target(2), 0.0);
        assert!((combat_intensity_target(3) - 1.0 / 7.0).abs() < 1e-6);
        assert!((combat_intensity_target(9) - 1.0).abs() < 1e-6);
        assert_eq!(combat_intensity_target(10), 1.0);
        assert_eq!(combat_intensity_target(40), 1.0);
    }

    #[test]
    fn smooth_value_half_life_convergence() {
        // One half-life covers half the distance.
        assert!((smooth_value(0.0, 1.0, 0.55, 0.55) - 0.5).abs() < 1e-6);
        // Repeated steps converge geometrically.
        let mut v = 0.0f32;
        for _ in 0..10 {
            v = smooth_value(v, 1.0, 0.55, 0.55);
        }
        assert!((v - (1.0 - 2.0f32.powi(-10))).abs() < 1e-5);
        // Degenerate inputs snap.
        assert_eq!(smooth_value(0.2, 0.9, 0.0, 0.55), 0.9);
        assert_eq!(smooth_value(0.2, 0.9, -1.0, 0.55), 0.9);
        assert_eq!(smooth_value(0.2, 0.9, 0.1, 0.0), 0.9);
    }

    #[test]
    fn intensity_bus_law() {
        let channels = AudioChannels::default();
        assert!((intensity_bus(&channels) - 0.8 * 0.42).abs() < 1e-6);
        assert!((intensity_volume(0.5, &channels) - 0.5 * 0.8 * 0.42).abs() < 1e-6);
        assert!(intensity_candidates(AreaId::Oasis).is_empty());
        assert_eq!(intensity_path(AreaId::Oasis), None);
        assert!(intensity_path(AreaId::Desert).is_some());
    }

    fn intensity_world(area: AreaId) -> World {
        let mut world = World::new();
        let mut time = SimTime::default();
        time.delta_secs = 1.0 / 30.0;
        world.insert_resource(time);
        ...
        world.insert_resource(LoopTransition::default());
        world.init_resource::<CombatIntensityState>();
        world
    }

    fn spawn_foe(world: &mut World, kind: EnemyKind) {
        world.spawn(Enemy {
            kind,
            score: 1,
            touch_damage: 1,
    ...
    #[test]
    fn intensity_layer_spawns_smooths_and_resets() {
        let mut world = intensity_world(AreaId::Desert);
        let mut schedule = Schedule::default();
        schedule.add_systems(update_combat_intensity_audio);
        schedule.run(&mut world);
        // Area entry spawns the layer at rest.
        let mut layers = world.query::<&CombatIntensityLayer>();
        assert_eq!(layers.iter(&world).count(), 1);
        assert_eq!(layers.iter(&world).next().unwrap().current, 0.0);

        // Ten ordinary foes pin the target at full; smoothing climbs.
        for _ in 0..10 {
            spawn_foe(&mut world, EnemyKind::Bandit);
        }
        for _ in 0..120 {
            schedule.run(&mut world);
        }
        let mut layers = world.query::<&CombatIntensityLayer>();
        let current = layers.iter(&world).next().unwrap().current;
        assert!(current > 0.95, "smoothed up, got {current}");

        // Campfire suppression drives the target back to silence.
        world.resource_mut::<LoopTransition>().campfire_active = true;
        for _ in 0..120 {
            schedule.run(&mut world);
        }
        let mut layers = world.query::<&CombatIntensityLayer>();
        let current = layers.iter(&world).next().unwrap().current;
        assert!(current < 0.05, "suppressed down, got {current}");

        // Area change respawns the layer; reset clears it.
        world.resource_mut::<Run>().area = AreaId::Oasis;
        schedule.run(&mut world);
        let mut layers = world.query::<&CombatIntensityLayer>();
        assert_eq!(layers.iter(&world).count(), 0);

        let mut reset = Schedule::default();
        reset.add_systems(reset_combat_intensity);
        reset.run(&mut world);
        assert_eq!(world.resource::<CombatIntensityState>().last_area, None);
    }
}
