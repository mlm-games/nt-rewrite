...
mod tests {
    ...
    #[test]
    fn splash_and_spiral_art_resolve() {
        ...
        for frame in 0..8 {
            assert!(
                assets.uv("images/sprLogo.png", frame).is_some(),
                "logo frame {frame} resolves"
            );
        }
        assert!(assets.uv("images/sprLogoGlow.png", 0).is_some());
        for stem in [
            ...
            assert!(assets.uv(stem, 0).is_some(), "{stem} resolves");
        }
    }

    #[test]
    fn esc_opens_pause_through_full_frame() {
        // End-to-end pause toggle: Esc edge -> feed -> schedule ->
        // overlay Pause, and the pause view composes.
        let mut app = App::new_with_seed(0x90A05E);
        let mut sched = Scheduler::new();
        sched.size = (1280, 720);
        let ctx = RenderContext::new();
        app.pause_edge = true;
        // Several 60 Hz frames: the 30 Hz fixed step (and its menu
        // systems) runs every other frame.
        for _ in 0..4 {
            let _view = root_view(
                &mut sched,
                &ctx,
        ...
        assert!(
            app.sim.world.resource::<crate::state::Paused>().0,
            "Esc pauses"
        );
        assert_eq!(
        ...
        assert_eq!(
            menu_overlay_kind(
                AppState::InGame,
                OverlayMenu::Pause,
                &app.sim.world.resource::<MenuState>(),
                false,
            ),
            Some(MenuOverlay::Pause)
        );
    ...
    #[test]
    fn boot_reaches_main_menu_through_view() {
        ...
        for _ in 0..500 {
            ...
            if *app.sim.world.resource::<AppState>() == AppState::MainMenu {
                break;
            }
        }
    ...
    #[test]
    fn sim_runs_at_30hz_headless() {
        let mut app = App::new();
        let ran = app.advance(Duration::from_secs_f64(1.0));
        assert_eq!(ran, 30, "one wall second = 30 fixed steps");
        let ticks = app.sim.world.resource::<TickCount>().0;
        assert_eq!(ticks, 30);
        let time = app.sim.world.resource::<repame_sim::SimTime>();
        assert!((time.elapsed_secs - 1.0).abs() < 1e-6);
    }

    #[test]
    fn camera_boots_snapped_at_gml_scale() {
        // GML boot parity: 240-tall view (reference `NT_CAM_SCALE`),
        // camera snapped on the first live frame
        // (`force_snap_camera_position`); GML has no zoom, so the live
        // frame re-derives the same scale every window.
        let mut app = App::new_with_seed(0xC0FFEE);
        assert!(app.gml_cam.snap);
        assert!((app.cam.units_per_pixel - NT_CAM_SCALE).abs() < 1e-6);
        // Two seconds of frames: the scale stays fixed and the look
        // point converges near the player (framed exactly, no
        // aim/POI/shake).
        let mut sched = Scheduler::new();
        sched.size = (1280, 720);
        let ctx = RenderContext::new();
        ...
        assert!(
            (app.cam.units_per_pixel - NT_CAM_SCALE).abs() < 1e-6,
            "fixed GML scale, left {}",
            app.cam.units_per_pixel
        );
        let pp = player_pos(&mut app.sim.world).expect("player alive");
        let lean = (app.cam.center - pp).length();
        assert!(
            lean < 24.0,
            "camera sits near the player, lean {lean:.1}px (48px swing = old law)"
        );
    }

    #[test]
    fn menu_overlay_selection_from_state() {
        let mut menu = MenuState::default();
        assert_eq!(
            menu_overlay_kind(AppState::Splash, OverlayMenu::None, &menu, false),
            Some(MenuOverlay::Splash)
        );
        assert_eq!(
            menu_overlay_kind(AppState::MainMenu, OverlayMenu::None, &menu, false),
            Some(MenuOverlay::MainMenu)
        );
        assert_eq!(
            menu_overlay_kind(AppState::Loading, OverlayMenu::None, &menu, false),
            Some(MenuOverlay::Loading)
        );
        assert_eq!(
            menu_overlay_kind(AppState::Title, OverlayMenu::None, &menu, false),
            Some(MenuOverlay::Title)
        );
        // In game, quiet: no overlay.
        assert_eq!(
            menu_overlay_kind(AppState::InGame, OverlayMenu::None, &menu, false),
            None
        );
        // Pause / settings / credits overlays.
        assert_eq!(
            menu_overlay_kind(AppState::InGame, OverlayMenu::Pause, &menu, false),
            Some(MenuOverlay::Pause)
        );
        assert_eq!(
            menu_overlay_kind(AppState::InGame, OverlayMenu::Settings, &menu, false),
            Some(MenuOverlay::Settings)
        );
        assert_eq!(
            menu_overlay_kind(AppState::InGame, OverlayMenu::Credits, &menu, false),
            Some(MenuOverlay::Credits)
        );
        // Mutation offer under a clear overlay.
        menu.mutation_count = 2;
        assert_eq!(
            menu_overlay_kind(AppState::InGame, OverlayMenu::None, &menu, false),
            Some(MenuOverlay::Mutation)
        );
        // Game over wins over everything in game.
        assert_eq!(
            menu_overlay_kind(AppState::InGame, OverlayMenu::Pause, &menu, true),
            Some(MenuOverlay::GameOver)
        );
    }

    #[test]
    fn nt_ui_font_registers_silkscreen_family() {
        ensure_nt_font();
        assert_eq!(
            repose_text::font_family_name(NT_UI_FONT).as_deref(),
            Some("Silkscreen"),
            "overlay Text requests the Silkscreen family"
        );
    }

    #[test]
    fn title_lines_follow_cursor_race() {
    ...
    #[test]
    fn hud_overlay_list_nonempty_in_game() {
        let mut app = App::new_with_seed(0x1234_5678);
        assert_eq!(
            app.sim.world.resource::<AppState>(),
            &AppState::InGame,
            "boot lands in a live run"
        );
        // GML HUD rows: HP `hp/max` + level + weapon ammo.
        let rows = hud_overlay_lines(&mut app.sim.world, [1280.0, 720.0]);
        assert!(!rows.is_empty(), "in-game HUD must list labels");
        assert!(
            rows.iter().any(|(l, _, _, _, _, _, _)| l.contains('/')),
            "HP hp/max row, got {rows:?}"
        );
        // Splash captions follow the boot mode (GML `Vlambeer/Draw_0`):
        // mode 0 shows the save note.
        let splash: Vec<String> = menu_overlay_lines(MenuOverlay::Splash, &mut app.sim.world);
        assert!(
            splash.iter().any(|l| l.contains("DO NOT TURN OFF")),
            "mode-0 save note, got {splash:?}"
        );
        for kind in [
            MenuOverlay::MainMenu,
            MenuOverlay::Loading,
            ...
            assert!(
                !menu_overlay_lines(kind, &mut app.sim.world).is_empty(),
                "{kind:?} renders lines"
            );
        }
    }
    ...
    #[test]
    fn no_assets_path_is_graceful() {
        let mut app = App::new_with_seed(42);
        assert!(!app.has_assets());
        assert!(
            app.load_assets_from(Path::new("/nonexistent-nt-assets-dir"))
                .is_err(),
            "missing catalog is an error, not a panic"
        );
        assert!(!app.has_assets(), "failed load keeps the placeholder path");
        assert!(
            !placeholder_instances(&mut app.sim.world).is_empty(),
            "placeholder layer covers the live run"
        );
        assert!(app.cam.center.is_finite());
    }

    #[test]
    fn shell_keys_stage_into_held_and_edges() {
        let mut app = App::new_with_seed(7);
        let key = |k: Key, t: KeyEventType| KeyEvent {
            key: k,
        ...
        app.handle_key(&key(Key::Character('w'), KeyEventType::Down));
        app.handle_key(&key(Key::Character('W'), KeyEventType::Up));
        assert!(!app.held.contains(&KeyCode::KeyW), "Up releases");
        app.handle_key(&key(Key::Character('a'), KeyEventType::Down));
        assert!(app.held.contains(&KeyCode::KeyA));
        app.handle_key(&key(Key::Escape, KeyEventType::Down));
        assert!(app.pause_edge);
        app.handle_key(&key(Key::Character('r'), KeyEventType::Down));
        assert!(app.restart_edge);
        app.handle_key(&key(Key::Enter, KeyEventType::Down));
        assert!(app.interact_edge);
    }

    #[test]
    fn click_feeds_aim_and_fire() {
        let mut app = App::new_with_seed(9);
        let pp = app
            .sim
            .world
            .query::<(&Pos, &Player)>()
            ...
            .0
            .0;
        app.clicks.push(StagedClick {
            world: pp + Vec2::new(10.0, 0.0),
            dp: [0.0, 0.0],
        });
        app.feed_input();
        let mut input = app.sim.world.resource_mut::<NtInput>();
        assert!(input.aim_axis.x > 0.9, "aim points at the click");
        assert!(input.take_fire_pressed(), "click pulses fire");
    }

    /// Canvas-dp position of a live-GUI point under the test viewport
    /// (1280x720 dp: GUI 426x240, k=3 dp per GUI px, no offset).
    fn gui_dp(gx: f32, gy: f32) -> [f32; 2] {
        [gx * 3.0, gy * 3.0]
    }

    #[test]
    fn pause_clicks_do_not_fire_or_confirm() {
        // Open pause menu: a click on empty space stages nothing (no
        // fire, no confirm), so a stray click can never resume
        // through a MENU press.
        let mut app = App::new_with_seed(9);
        ...
        *app.sim.world.resource_mut::<OverlayMenu>() = OverlayMenu::Pause;
        let pp = app
            ...
            .0
            .0;
        app.hover = Some(pp + Vec2::new(50.0, 0.0));
        // Empty space (GUI 160,120: between the button columns/rows).
        app.clicks.push(StagedClick {
            world: pp,
            dp: gui_dp(160.0, 120.0),
        });
        app.feed_input();
        let mut input = app.sim.world.resource_mut::<NtInput>();
        assert_eq!(input.aim_axis, Vec2::ZERO, "hover frozen while paused");
        assert!(!input.take_fire_pressed(), "no fire while paused");
        assert!(
            !input.take_interact_pressed(),
            "bare pause click is not a confirm"
        );
        assert_eq!(
            app.sim.world.resource::<MenuState>().pause_confirm,
            None,
            "empty click arms nothing"
        );
    }

    #[test]
    fn pause_click_routes_to_button() {
        // Position routing: MENU arms quit-confirm, CONTINUE resumes,
        // confirm BACK/QUIT unwind/quit. Drives `feed_input` directly
        // (no repose hit-testing involved).
        let mut app = App::new_with_seed(9);
        app.sim.world.resource_mut::<crate::state::Paused>().0 = true;
        *app.sim.world.resource_mut::<OverlayMenu>() = OverlayMenu::Pause;
        // MENU at GUI (45,176).
        app.clicks.push(StagedClick {
            world: Vec2::ZERO,
            dp: gui_dp(45.0, 181.0),
        });
        app.feed_input();
        assert_eq!(
            app.sim.world.resource::<MenuState>().pause_confirm,
            Some(0),
            "MENU click arms the quit confirm"
        );
        // BACK at GUI (52,192) cancels it.
        app.clicks.push(StagedClick {
            world: Vec2::ZERO,
            dp: gui_dp(52.0, 197.0),
        });
        app.feed_input();
        assert_eq!(
            app.sim.world.resource::<MenuState>().pause_confirm,
            None,
            "BACK click cancels the confirm"
        );
        // CONTINUE at GUI (vw-78,208) = (348,208) at 426-wide resumes.
        app.clicks.push(StagedClick {
            world: Vec2::ZERO,
            dp: gui_dp(348.0, 213.0),
        });
        app.feed_input();
        assert_eq!(
            *app.sim.world.resource::<OverlayMenu>(),
            OverlayMenu::None,
            "CONTINUE click resumes"
        );
    }

    #[test]
    fn pause_buttons_stage_actions() {
        // Button map covers the GML pause layout (main + quit/restart
        // confirms); staged actions apply through `feed_input`.
        assert!(matches!(
            menu_button_action(MenuOverlay::Pause, "CONTINUE", None),
            Some(UiAction::Resume)
        ));
        assert!(matches!(
            menu_button_action(MenuOverlay::Pause, "MENU", None),
            Some(UiAction::ShowPauseConfirm(0))
        ));
        assert!(matches!(
            menu_button_action(MenuOverlay::Pause, "RETRY", None),
            Some(UiAction::ShowPauseConfirm(1))
        ));
        assert!(matches!(
            menu_button_action(MenuOverlay::Pause, "SETTINGS", None),
            Some(UiAction::OpenSettings)
        ));
        assert!(menu_button_action(MenuOverlay::Pause, "PAUSED", None).is_none());
        assert!(matches!(
            menu_button_action(MenuOverlay::Pause, "BACK", Some(0)),
            Some(UiAction::CancelPauseConfirm)
        ));
        assert!(matches!(
            menu_button_action(MenuOverlay::Pause, "QUIT", Some(0)),
            Some(UiAction::ConfirmPause(0))
        ));
        assert!(matches!(
            menu_button_action(MenuOverlay::Pause, "RETRY", Some(1)),
            Some(UiAction::ConfirmPause(1))
        ));
        assert!(matches!(
            menu_button_action(MenuOverlay::Settings, "BACK", None),
            Some(UiAction::SettingsBack)
        ));
        assert!(matches!(
            menu_button_action(MenuOverlay::Credits, "BACK", None),
            Some(UiAction::CloseOverlay)
        ));

        // Staged MENU arms the quit confirm; staged CONTINUE resumes.
        let mut app = App::new_with_seed(9);
        app.sim.world.resource_mut::<crate::state::Paused>().0 = true;
        *app.sim.world.resource_mut::<OverlayMenu>() = OverlayMenu::Pause;
        app.stage_menu_action(UiAction::ShowPauseConfirm(0));
        app.feed_input();
        assert_eq!(
            app.sim.world.resource::<MenuState>().pause_confirm,
            Some(0),
            "MENU arms the quit confirm"
        );
        app.stage_menu_action(UiAction::Resume);
        app.feed_input();
        assert_eq!(
            *app.sim.world.resource::<OverlayMenu>(),
            OverlayMenu::None,
    ...
    #[test]
    fn camera_freezes_while_paused() {
        // GML `BackCont` is dead while paused: fixed ticks must not
        // move the look point, whatever the cursor does.
        let mut app = App::new_with_seed(0xC0FFEE);
        app.advance(Duration::from_secs_f64(1.0 / SIM_HZ));
        let (x, y) = (app.gml_cam.x, app.gml_cam.y);
        app.sim.world.resource_mut::<crate::state::Paused>().0 = true;
        *app.sim.world.resource_mut::<OverlayMenu>() = OverlayMenu::Pause;
        app.hover = Some(Vec2::new(10_000.0, -10_000.0));
        app.advance(Duration::from_secs_f64(2.0));
        assert_eq!((app.gml_cam.x, app.gml_cam.y), (x, y));
    }

    #[test]
    fn camera_aim_lean_caps_at_48px() {
        // Bevy `MAX_LOOK` parity: cursor at the window edge must not
        // drag the look point more than 48 px off the player.
        let mut app = App::new_with_seed(0xC0FFEE);
        let pp = player_pos(&mut app.sim.world).expect("player alive");
        app.hover = Some(pp + Vec2::new(10_000.0, 0.0));
        app.advance(Duration::from_secs_f64(5.0));
        let [vw, vh] = app.view_world_size;
        let center = Vec2::new(app.gml_cam.x + vw * 0.5, app.gml_cam.y + vh * 0.5);
        let lean = (center - pp).length();
        assert!(
            lean <= crate::render::CAM_MAX_LOOK + 2.0,
            "lean capped near 48px, got {lean:.1}px"
        );
        assert!(lean > 8.0, "lean still follows the mouse, got {lean:.1}px");
    }
    ...
    fn camera_snaps_on_first_fixed_tick() {
        ...
        assert!(app.gml_cam.snap);
        app.advance(Duration::from_secs_f64(1.0 / SIM_HZ));
        assert!(!app.gml_cam.snap, "snap consumed by the first tick");
        let pp = player_pos(&mut app.sim.world).expect("player alive");
        let [vw, vh] = app.view_world_size;
        let center = Vec2::new(app.gml_cam.x + vw * 0.5, app.gml_cam.y + vh * 0.5);
        assert!(
            (center - pp).length() < 2.0,
            "look point on the player, off by {:.1}px",
            (center - pp).length()
        );
    }
    ...
    #[test]
    fn hover_aims_every_frame_without_clicks() {
        let mut app = App::new_with_seed(11);
        let pp = app
            .sim
            .world
            .query::<(&Pos, &Player)>()
            ...
            .0
            .0;
        // Cursor left of the player: aim follows with no click, no fire.
        app.hover = Some(pp + Vec2::new(-30.0, 0.0));
        app.feed_input();
        let mut input = app.sim.world.resource_mut::<NtInput>();
        assert!(input.aim_axis.x < -0.9, "hover steers aim left");
        assert!(!input.take_fire_pressed(), "hover never fires");
        drop(input);
    ...
    #[test]
    fn staged_gamepad_and_touch_feed_verbatim() {
        let mut app = App::new_with_seed(11);
        // Right stick steers aim (stick wins over hover).
        app.hover = Some(Vec2::new(10_000.0, 0.0));
        app.stage_gamepad(GamepadState {
            right_stick: Vec2::new(0.0, 1.0),
            ..GamepadState::default()
        });
        app.feed_input();
        let input = app.sim.world.resource::<NtInput>();
        assert!(input.aim_axis.y > 0.9, "pad stick steers aim");
        // Staged pads drain (take-once staging, not sticky): with the
        // cursor cleared the axis falls back to zero.
        app.hover = None;
        app.feed_input();
        let input = app.sim.world.resource::<NtInput>();
        assert_eq!(input.aim_axis, Vec2::ZERO);
        // Right-half touch fires through the feed (touch-down stages
        // a fresh contact; the feed synthesizes the contact list).
        app.view_width = 1280.0;
        app.touch_down(7, Vec2::new(960.0, 400.0));
        app.feed_input();
        let mut input = app.sim.world.resource_mut::<NtInput>();
        assert!(input.take_fire_pressed(), "touch fires");
        drop(input);
        // Held contact keeps holding fire on later ticks without new
        // edges (bevy `touches.iter()` parity).
        app.feed_input();
        let input = app.sim.world.resource::<NtInput>();
        assert!(input.fire_held, "held touch keeps firing");
        let mut input = app.sim.world.resource_mut::<NtInput>();
        ...
        app.feed_input();
        let input = app.sim.world.resource::<NtInput>();
        assert!(!input.fire_held, "released touch stops");
    }

    #[test]
    fn root_view_builds_headless_without_assets() {
        let mut app = App::new_with_seed(0xBEEF);
        let mut sched = Scheduler::new();
        sched.size = (1280, 720);
        let ctx = RenderContext::new();
        let _view = root_view(
        ...
        assert_eq!(app.last_bg_alpha(), 1.0, "live gameplay selects vortex bg");
