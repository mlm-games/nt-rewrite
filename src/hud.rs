...
mod tests {
    use super::*;
    use crate::comps_a::{AimDir, Score, SelectedCharacter};
    use crate::comps_b::BossBrain;
    use crate::data::{AmmoKind, RaceId, WeaponId};
    use crate::spatial::Pos;
    use crate::time::{GTimer, TimerMode};
    use glam::Vec2;

    fn spawn_player(world: &mut World, hp: i32, max: i32) -> Entity {
        let mut player = Player::default();
        player.level = 3;
        player.rads = 25;
        player.next_level_rads = 60;
        world
            .spawn((
                player,
                Health {
    ...
    #[test]
    fn reflects_damage_and_level_progress() {
        let mut world = base_world();
        spawn_player(&mut world, 3, 8);
        let hud = sync_hud_state(&world);
        assert_eq!((hud.hp, hud.max_hp), (3, 8));
        assert_eq!((hud.rads, hud.max_rads, hud.level), (25, 60, 3));
        assert!(!hud.game_over);
    }

    #[test]
    fn reflects_ammo_and_melee_slot_law() {
        let mut world = base_world();
        let e = spawn_player(&mut world, 8, 8);
        // Slot 0 melee (wrench = AmmoType::None) => weapon_ammo[0] == -1.
        world.get_mut::<Inventory>(e).unwrap().weapons[0] = WeaponId::WRENCH;
        let hud = sync_hud_state(&world);
        assert_eq!(hud.ammo, [0, 40, 8, 0, 0, 0]);
        assert_eq!(hud.weapon_ammo[0], -1, "melee slot shows no ammo");
        assert_eq!(hud.weapon_ammo[1], 8, "shotgun shells");
    }

    #[test]
    fn reflects_weapon_switch() {
        let mut world = base_world();
        let e = spawn_player(&mut world, 8, 8);
        world.get_mut::<Inventory>(e).unwrap().current = 1;
        let hud = sync_hud_state(&world);
        assert_eq!(hud.current_weapon, 1);
        assert_eq!(hud.weapons, vec!["REVOLVER".to_string(), "SHOTGUN".to_string()]);
    }

    #[test]
    fn reflects_rad_gain() {
        let mut world = base_world();
        let e = spawn_player(&mut world, 8, 8);
        world.get_mut::<Player>(e).unwrap().rads += 35;
        let hud = sync_hud_state(&world);
        assert_eq!(hud.rads, 60);
        assert_eq!(hud.max_rads, 60);
    }

    #[test]
    fn reflects_boss_presence_and_picks_max_hp() {
        let mut world = base_world();
        spawn_player(&mut world, 8, 8);
        assert!(sync_hud_state(&world).boss_name.is_empty());
        let spawn_boss = |world: &mut World, kind: crate::data::EnemyKind, hp: i32, max: i32| {
            world.spawn((
                Enemy {
                    kind,
                    score: 0,
                ...
                BossBrain::new(kind, Vec2::ZERO),
                Pos(Vec2::ZERO),
            ));
        };
        spawn_boss(
            &mut world,
            crate::data::EnemyKind::BigBandit,
            50,
            100,
        );
        spawn_boss(&mut world, crate::data::EnemyKind::Mom, 10, 500);
        let hud = sync_hud_state(&world);
        assert_eq!(hud.boss_name, "Mom");
        assert_eq!((hud.boss_hp, hud.boss_max), (10, 500));
    }

    #[test]
    fn reflects_mutation_choices_and_death_ids() {
        use crate::data::MutationId::*;
        let mut world = base_world();
        spawn_player(&mut world, 8, 8);
        world.insert_resource(PendingMutation {
            choices: vec![RhinoSkin, StrongSpirit],
        });
        let hud = sync_hud_state(&world);
        assert_eq!(
            hud.mutation_choices,
            vec![
                "Rhino Skin - +4 max HP".to_string(),
                "Strong Spirit - Prevents death, once".to_string(),
            ]
        );
        assert_eq!(hud.mutation_choice_ids, vec![1, 27]);
        assert!(hud.death_mutation_ids.is_empty());

        world.remove_resource::<PendingMutation>();
        world.resource_mut::<Run>().game_over = true;
        let hud = sync_hud_state(&world);
        assert!(hud.game_over);
        assert!(hud.mutation_choices.is_empty());
        assert!(hud.death_mutation_ids.is_empty(), "player has no mutations");
    }

    #[test]
    fn reflects_idpd_warning_and_toast() {
        let mut world = base_world();
        spawn_player(&mut world, 8, 8);
        assert!(!sync_hud_state(&world).idpd_warning);
        world.resource_mut::<Toast>().show("IDPD INCOMING");
        world.resource_mut::<IdpdRaidState>().pending_wave =
            Some(crate::comps_b::RaidWave::Light);
        let hud = sync_hud_state(&world);
        assert!(hud.idpd_warning);
        assert_eq!(hud.toast, "IDPD INCOMING");
        assert!(hud.toast_timer > 0.0);
    }

    #[test]
    fn reflects_kills_score_save_and_crown() {
        let mut world = base_world();
        let e = spawn_player(&mut world, 8, 8);
        world.get_mut::<Player>(e).unwrap().crown = CrownKind::Guns;
        world.resource_mut::<Run>().total_kills = 12;
        world.resource_mut::<Score>().0 = 345;
        let mut save = world.resource_mut::<SaveData>();
        save.high_score = 999;
        save.best_floor = 7;
        save.total_kills = 1234;
        drop(save);
        let hud = sync_hud_state(&world);
        assert_eq!(hud.kills, 12);
        assert_eq!(hud.score, 345);
        assert_eq!(hud.high_score, 999);
        assert_eq!(hud.best_floor, 7);
        assert_eq!(hud.total_kills, 1234);
        assert_eq!(hud.crown, "GUNS");
        assert_eq!(hud.ability, "Flip");
        assert!(hud.ability_ready);
    }

    #[test]
    fn no_run_clears_run_scoped_fields() {
        let world = World::new();
        let hud = sync_hud_state(&world);
        assert!(!hud.game_over);
        assert_eq!(hud.loop_count, 0);
        assert!(hud.toast.is_empty() && hud.boss_name.is_empty());
        assert!(hud.mutation_choices.is_empty() && hud.death_mutation_ids.is_empty());
    }

    #[test]
    fn run_clock_and_area_strings_follow_gml() {
        // `M:SS.CC` at 30 steps/s: 0 → 0:00.00, 30 → 0:01.00,
        // 1899 → 1:03.30.
        assert_eq!(run_timer_string(0), "0:00.00");
        assert_eq!(run_timer_string(30), "0:01.00");
        assert_eq!(run_timer_string(1899), "1:03.30");
        // Map names: desert 1-2, secrets `N-?`, vault `???`, loops `L#`.
        let desert = Run {
            floor: 2,
            floor_in_area: 2,
            ..Run::default()
        };
        assert_eq!(run_area_string(&desert), "1-2");
        let looped = Run {
            loop_count: 2,
            ..Run::default()
        };
        assert_eq!(run_area_string(&looped), "1-1 L2");
    }

    #[test]
    fn reset_hud_state_is_default_cleared() {
        // Bevy `reset_hud_flags` clears in place: flags/toast/boss/loop
        // reset, live run data (hp, weapons, ammo, level) preserved.
        let mut hud = HudState {
            hp: 6,
            level: 3,
            weapons: vec!["Shotgun".to_string()],
            weapon_ids: vec![crate::data::WeaponId::SHOTGUN],
            weapon_cursed: vec![false],
            ammo: [1, 2, 3, 4, 5, 6],
            loop_count: 2,
            boss_hp: 50,
            boss_max: 100,
            boss_name: "Big Bandit".to_string(),
            toast: "hi".to_string(),
            toast_timer: 1.5,
            game_over: true,
            mutation_choices: vec!["a".to_string()],
            ..HudState::default()
        };
        reset_hud_state(&mut hud);
        assert_eq!(hud.loop_count, 0);
        assert!(hud.toast.is_empty() && hud.boss_name.is_empty());
        assert!(hud.mutation_choices.is_empty() && hud.death_mutation_ids.is_empty());
        assert!(!hud.game_over && hud.toast_timer == 0.0);
        assert_eq!((hud.boss_hp, hud.boss_max), (0, 0));
        assert_eq!((hud.hp, hud.level), (6, 3));
        assert_eq!(hud.weapons, vec!["Shotgun".to_string()]);
        assert_eq!(hud.ammo, [1, 2, 3, 4, 5, 6]);
    }

    #[test]
    fn fainted_bars_thread_revive_alarms() {
        use crate::comps_b::Revive;
        let mut world = base_world();
        spawn_player(&mut world, 8, 8);
        assert!(sync_hud_state(&world).fainted_bars.is_empty());
        world.spawn((
        ...
        let mut cleared = hud.clone();
        reset_hud_state(&mut cleared);
        assert!(cleared.fainted_bars.is_empty());
    }

    #[test]
    fn ammo_kind_index_matches_inventory_layout() {
        // Guard the weapon_ammo indexing law: AmmoKind discriminants line up
        // with the Inventory.ammo slots bevy used.
        assert_eq!(AmmoKind::Bullets as usize, 1);
        assert_eq!(AmmoKind::Energy as usize, 5);
        let _ = RaceId::Fish;
    }
}
