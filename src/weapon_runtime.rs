...
mod tests {
    ...
    #[test]
    fn fire_push_table_parity() {
        // Most guns have NO self-push (the old `recoil * 18` shoved
        // everything); melee lunges per weapon.
        assert_eq!(gml_fire_push_px_s("REVOLVER"), 0.0);
        assert_eq!(gml_fire_push_px_s("GRENADE LAUNCHER"), 0.0);
        assert_eq!(gml_fire_push_px_s("SHOTGUN"), 0.0);
        assert_eq!(gml_fire_push_px_s("DOUBLE SHOTGUN"), 60.0);
        assert_eq!(gml_fire_push_px_s("SUPER PLASMA CANNON"), 480.0);
        assert_eq!(gml_fire_push_px_s("SCREWDRIVER"), -120.0);
        assert_eq!(gml_fire_push_px_s("BLACK SWORD"), -240.0);
        assert_eq!(gml_melee_wkick("SCREWDRIVER"), -8.0);
        assert_eq!(gml_melee_wkick("BLACK SWORD"), -7.0);
        assert_eq!(gml_melee_wkick("ENERGY HAMMER"), -3.0);
        assert_eq!(gml_melee_wkick("WRENCH"), -4.0);
    }

    #[test]
    fn revolver_def_parity() {
        let def = weapon_runtime_def(id_by_name("REVOLVER"));
        assert_eq!(def.damage, 3);
        assert_eq!(def.pellets, 1);
        assert_eq!(def.speed, 480.0);
        assert!((def.cooldown - 6.0 / 30.0).abs() < f32::EPSILON);
        assert_eq!(def.ammo, AmmoKind::Bullets);
        assert!(!def.automatic);
        assert!(!def.explosive);
        assert!(def.melee.is_none());

        let rt = weapon_runtime(id_by_name("REVOLVER"));
        assert_eq!(rt.damage, 3);
        assert_eq!(rt.pellets, 1);
        assert_eq!(rt.cooldown_frames, 6);
        assert!(matches!(rt.projectile_kind, ProjectileKind::Bullet));
    }

    #[test]
    fn shotgun_fires_spread_volley() {
        let def = weapon_runtime_def(id_by_name("SHOTGUN"));
        assert!(def.pellets > 1, "pellets={}", def.pellets);
        assert_eq!(def.pellets, 7);
        assert!(def.spread > 0.3, "spread={}", def.spread);
        assert_eq!(def.ammo, AmmoKind::Shells);
    }

    #[test]
    fn sledgehammer_is_melee() {
        let def = weapon_runtime_def(id_by_name("SLEDGEHAMMER"));
    ...
    #[test]
    fn grenade_launcher_is_explosive() {
        let def = weapon_runtime_def(id_by_name("GRENADE LAUNCHER"));
        assert!(def.explosive);
        assert_eq!(def.damage, 15);
        assert_eq!(def.ammo, AmmoKind::Explosives);
        assert_eq!(def.bounces, 4);
    }

    #[test]
    fn crossbow_fires_bolts() {
        let def = weapon_runtime_def(id_by_name("CROSSBOW"));
        assert_eq!(def.ammo, AmmoKind::Bolts);
        assert_eq!(def.damage, 20);
        assert_eq!(def.pellets, 1);
    }

    #[test]
    fn all_registry_ids_build_without_panic() {
        assert!(WEAPONS.len() >= 126, "len={}", WEAPONS.len());
        assert_eq!(WEAPONS.len(), MAXWEP + 1);
        let mut built = 0;
        for meta in WEAPONS.iter() {
            let def = weapon_runtime_def(WeaponId(meta.id));
            if meta.id == WeaponId::NONE.0 {
                // Verbatim bevy behavior: id 0 returns the legacy
                // `weapon_def(WeaponKind::None)` seed ("None"), not the
                // empty registry row.
                assert_eq!(def.name, "None");
                assert_eq!(def.damage, 0);
                assert_eq!(def.pellets, 0);
            } else {
                assert_eq!(def.name, meta.wep_name, "id {}", meta.id);
                assert_eq!(def.ammo_cost, i32::from(meta.wep_cost), "id {}", meta.id);
                assert_eq!(def.automatic, meta.wep_auto, "id {}", meta.id);
                assert!(def.cooldown > 0.0, "id {}", meta.id);
            }
            built += 1;
        }
        assert!(built >= 126, "built={built}");
    }

    #[test]
    fn golden_variant_tuning_spot_check() {
        ...
        assert_eq!(golden.color, [1.0, 0.82, 0.2]);

        let revolver = weapon_runtime_def(id_by_name("REVOLVER"));
        let golden_rev = weapon_runtime_def(id_by_name("GOLDEN REVOLVER"));
        assert_eq!(golden_rev.damage, revolver.damage);
        assert_eq!(golden_rev.pellets, revolver.pellets);
        assert_eq!(
    ...
    fn sleep_heavy_gt_auto() {
        ...
        assert!(heavy > auto, "heavy {heavy} should exceed auto {auto}");
        assert!(shotgun > auto, "shotgun {shotgun} should exceed auto {auto}");
        assert!(weapon_sleep_secs(WeaponId::NONE) == 0.0);
    }
}
