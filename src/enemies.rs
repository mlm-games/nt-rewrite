...
mod tests {
    use super::*;
    use crate::setup::spawn_player;
    use repame_anim::{AnimCatalog, AtlasDesc};

    #[test]
    fn elite_blocker_deflects_and_eats_shots_then_poofs() {
        let mut world = World::new();
        base_resources(&mut world);
        let owner = world
            .spawn((
                GameCleanup,
                Team::Enemy,
                Enemy {
                    kind: EnemyKind::EliteShielder,
                    score: 0,
                    touch_damage: 0,
                    rad_drop: 0,
                    drop_chance: 0,
        ...
        let mk_shot = |world: &mut World, typ: u8| {
            world
                .spawn((
                    GameCleanup,
                    LevelCleanup,
                    ...
                    ProjectileTyp(typ),
                    Velocity(glam::Vec2::new(-60.0, 0.0)),
                    Pos(glam::Vec2::new(210.0, 0.0)),
                ))
                .id()
        };
        let deflect = mk_shot(&mut world, 1);
        let eat = mk_shot(&mut world, 2);
        let mut sched = bevy_ecs::schedule::Schedule::default();
        sched.add_systems(tick_elite_blockers);
        sched.run(&mut world);
        // Typ 1 re-teams to the IDPD side and flies off.
        assert_eq!(
            world.get::<Team>(deflect),
            Some(&Team::Enemy),
            "deflectable shot turns"
        );
        assert!(
            world.get::<Velocity>(deflect).unwrap().0.x > 0.0,
            "deflected away from the shield"
        );
        // Typ 2 is destroyed.
        assert!(world.get_entity(eat).is_err(), "destructible shot dies");
        // Owner pinned to the anchor.
        let op = world.get::<Pos>(owner).unwrap().0;
        assert_eq!(op, glam::Vec2::new(200.0, 0.0), "creator pinned");
        // Expiry poofs the anchor away.
        for _ in 0..70 {
            sched.run(&mut world);
        }
        assert_eq!(
            world.query::<&EliteBlocker>().iter(&world).count(),
            0,
            "anchor expires"
        );
    }

    fn elite_brain(attack_secs: f32) -> EnemyBrain {
        EnemyBrain {
            speed: 100.0,
            accel: 900.0,
            preferred_range: 100.0,
            shoot_range: 160.0,
            attack: GTimer::from_seconds(attack_secs, TimerMode::Once),
            burst_left: 0,
            burst_timer: GTimer::from_seconds(0.01, TimerMode::Once),
            dash: 0.0,
            strafe_dir: 0.0,
            strafe_timer: GTimer::from_seconds(1.0, TimerMode::Once),
            melee: GTimer::from_seconds(0.01, TimerMode::Once),
            walk: 0.0,
            ammo: 0,
            slash_delay: 0.0,
            gunangle: 0.0,
    ...
    fn spawn_elite(world: &mut World, kind: EnemyKind, at: glam::Vec2) {
        world.spawn((
            GameCleanup,
            LevelCleanup,
            Team::Enemy,
    ...
    #[test]
    fn elite_inspector_slashes_up_close() {
        let mut world = World::new();
        base_resources(&mut world);
        world.spawn((
            Player::default(),
        ...
        spawn_elite(&mut world, EnemyKind::EliteInspector, glam::Vec2::new(30.0, 0.0));
        let mut sched = bevy_ecs::schedule::Schedule::default();
        sched.add_systems(tick_elite_inspectors);
        // Let init run, then force the baton-slash tick (`alarm[2]`).
        sched.run(&mut world);
        sched.run(&mut world);
        {
            let mut q = world.query::<(&Enemy, &mut EnemyBrain)>();
            let (_, mut brain) = q
                .iter_mut(&mut world)
                .find(|(e, _)| e.kind == EnemyKind::EliteInspector)
                .expect("inspector");
            brain.gunangle = std::f32::consts::PI;
            brain.slash_delay = 0.05;
        }
        for _ in 0..5 {
            sched.run(&mut world);
        }
        let slash = world.query::<&Projectile>().iter(&world).count();
        assert_eq!(slash, 1, "inspector baton slash fired");
        // Freeze accrues while the target lives (`Other_10`).
        for _ in 0..50 {
            sched.run(&mut world);
        }
    ...
    #[test]
    fn elite_shielder_bursts_plasma_in_range() {
        let mut world = World::new();
        base_resources(&mut world);
        world.spawn((
            Player::default(),
        ...
        let mut sched = bevy_ecs::schedule::Schedule::default();
        sched.add_systems(tick_elite_shielders);
        // Let init run, then force a loaded burst tick (`alarm[2]`).
        sched.run(&mut world);
        sched.run(&mut world);
        {
            let mut q = world.query::<(&Enemy, &mut EnemyBrain)>();
            let (_, mut brain) = q
                .iter_mut(&mut world)
                .find(|(e, _)| e.kind == EnemyKind::EliteShielder)
                .expect("shielder");
            brain.ammo = 6;
            brain.slash_delay = 0.05;
        }
        ...
        let plasma = world.query::<&Projectile>().iter(&world).count();
        assert_eq!(plasma, 1, "loaded burst tick fires one plasma");
        // Freeze accrues +3/tick while the target lives (`Other_10`).
        for _ in 0..50 {
            sched.run(&mut world);
        }
        let mut q = world.query::<(&Enemy, &EnemyBrain)>();
    ...
    #[test]
    fn throne_ball_sprays_after_stall_then_dies() {
        let mut world = World::new();
        base_resources(&mut world);
        // Player to aim at while stalled.
        world.spawn((
            Player::default(),
            Team::Player,
            Pos(glam::Vec2::new(200.0, 0.0)),
        ...
        let ball = world
            .spawn((
                ...
                Projectile {
                    damage: 12,
                    life: GTimer::from_seconds(10.0, TimerMode::Once),
                    radius: 10.0,
                    knockback: 100.0,
                    explosive: false,
                    source: None,
            ...
            .id();
        let mut sched = bevy_ecs::schedule::Schedule::default();
        sched.add_systems(tick_throne_balls);
        // Young stall: aims, no spray yet.
        for _ in 0..10 {
            sched.run(&mut world);
        }
        assert!(world.get_entity(ball).is_ok(), "ball alive while aiming");
        assert_eq!(
            world.query::<&Projectile>().iter(&world).count(),
            1,
            "no spray before 15 stalled ticks"
        );
        // Old stall: one Horror bolt per tick.
        for _ in 0..10 {
            sched.run(&mut world);
        }
        let spray = world.query::<&Projectile>().iter(&world).count();
        assert!(spray > 5, "stall sprays bolts, got {spray}");
        // Past the timeout cap the ball is gone.
        for _ in 0..40 {
            sched.run(&mut world);
        }
        assert!(world.get_entity(ball).is_err(), "timeout despawns");
    }

    #[test]
    fn corpses_drift_with_friction_then_despawn() {
        let mut world = World::new();
        ...
        let e = world
            ...
            .id();
        let mut sched = bevy_ecs::schedule::Schedule::default();
        sched.add_systems(tick_corpses);
        sched.run(&mut world);
        let v = world.get::<Velocity>(e).unwrap().0.x;
        assert!(v < 100.0 && v > 0.0, "friction bleeds slide, got {v}");
        assert!(world.get::<Pos>(e).unwrap().0.x > 0.0);
        for _ in 0..4 {
            sched.run(&mut world);
        }
        assert!(world.get_entity(e).is_err(), "expiry despawns");
    }
    ...
    /// Floor mask covering the central ±640 px (all LOS rays in these
    /// tests stay inside walkable cells).
    fn open_mask() -> FloorMask {
    ...
    fn base_resources(world: &mut World) {
        world.insert_resource(sim_time());
        world.insert_resource(open_mask());
        world.insert_resource(Run::default());
        world.insert_resource(empty_catalog());
        world.insert_resource(Euphoria(false));
        world.insert_resource(Trauma::default());
        world.insert_resource(Queue::<AudioCue>::default());
    }

    #[test]
    fn proto_statue_charges_and_phases_with_idpd_waves() {
        let mut world = World::new();
        base_resources(&mut world);
        let mut player = Player::default();
        player.rads = 30;
        world.spawn((player, Team::Player, Pos(glam::Vec2::ZERO)));
        let statue = world
            .spawn((
                GameCleanup,
                LevelCleanup,
                ...
                ProtoGuardian::default(),
            ))
            .id();
        let mut sched = bevy_ecs::schedule::Schedule::default();
        sched.add_systems(tick_proto_statues);
        sched.run(&mut world);
        sched.run(&mut world);
        let st = world.get::<ProtoGuardian>(statue).expect("guardian");
        assert!(st.charged, "rich statue charges");
        assert_eq!(world.get::<Health>(statue).unwrap().hp, 60, "hp halved");
        // Halving drops below 70%, so the phase wave fires at once too
        // (GML-identical): 4 portals (popolevel-gated kinds vary).
        assert!(st.phased, "phase follows the halving");
        assert!(
            world.query::<&PendingEnemySpawn>().iter(&world).count() >= 4,
            "charge + phase raise IDPD portals"
        );
        assert_eq!(
            world.resource::<Run>().popolevel, 4,
            "one popolevel per portal"
        );
        // Latched: no further waves on later ticks.
        sched.run(&mut world);
        assert_eq!(
            world.resource::<Run>().popolevel, 4,
            "waves fire once"
    ...
    #[test]
    fn spawn_enemy_at_applies_table_stats_and_brains() {
        let mut world = World::new();
        let catalog = empty_catalog();
        let (bandit, boss, van, scorp);
        {
            let mut cmds = world.commands();
            bandit = spawn_enemy_at(
                &mut cmds,
                &catalog,
                EnemyKind::Bandit,
                glam::Vec2::ZERO,
        ...
        let bandit_def = enemy_def(EnemyKind::Bandit);
        let health = world.get::<Health>(bandit).unwrap();
        assert_eq!((health.hp, health.max), (bandit_def.hp, bandit_def.hp));
        let brain = world.get::<EnemyBrain>(bandit).unwrap();
        assert_eq!(brain.preferred_range, bandit_def.preferred_range);
        assert_eq!(brain.shoot_range, bandit_def.shoot_range);
        assert_eq!(brain.ammo, 0);
        assert!(world.get::<BossBrain>(bandit).is_none());

        // GML loop law drives HP (difficulty only feeds speed now):
        // loop-1 ScrapBoss → ceil(300 * (1 + 1/1.2)) = 550.
        assert_eq!(world.get::<Health>(boss).unwrap().hp, 550);
        assert!(world.get::<BossBrain>(boss).is_some());

        // IDPD van rides both brains.
        assert!(world.get::<IdpdVanBrain>(van).is_some());
        assert!(world.get::<IdpdShieldUnit>(van).is_some());
        // Ammo table.
        assert_eq!(world.get::<EnemyBrain>(scorp).unwrap().ammo, 10);
    }

    #[test]
    fn show_enemy_fire_repaths_to_fire_strip() {
        use repame_anim::AtlasDesc;
        // Bevy `play_fire`: live anim repaths to the fire strip as a
        // 0.25 s oneshot; unknown strips and hurting enemies no-op.
        let catalog = AnimCatalog::from_json(
            r#"{"images/sprBanditBossIdle.png": {"frames": 2, "w": 32, "h": 32, "fps": 8.0, "xorigin": 16.0, "yorigin": 16.0},
                "images/sprBanditBossFire.png": {"frames": 2, "w": 32, "h": 32, "fps": 8.0, "xorigin": 16.0, "yorigin": 16.0}}"#,
            AtlasDesc {
                size: 256,
                max_pages: 1,
            padding: 0,
        ...
        let e = world
            .spawn((crate::anim::SpriteAnim::new(
                "images/sprBanditBossIdle.png",
                world
                    .resource::<repame_anim::AnimCatalog>()
                    .def("images/sprBanditBossIdle.png")
                    .unwrap(),
            ),))
            .id();
        // Drive through a one-shot system (anim + commands together).
        let mut sched = Schedule::default();
        sched.add_systems(
            move |mut commands: Commands,
                  catalog: Res<repame_anim::AnimCatalog>,
                  mut q: Query<(Entity, &mut crate::anim::SpriteAnim)>| {
                for (ee, mut anim) in &mut q {
                    if ee == e {
                        show_enemy_fire(
                            &mut commands,
                            &catalog,
        ...
        sched.run(&mut world);
        let anim = world.get::<crate::anim::SpriteAnim>(e).unwrap();
        assert_eq!(anim.path, "images/sprBanditBossFire.png");
        assert!(anim.oneshot);
        assert!(world.get::<crate::comps_b::FireAnim>(e).is_some());
        // Hurting enemies keep idle.
        let f = world
            .spawn((crate::anim::SpriteAnim::new(
                "images/sprBanditBossIdle.png",
                world
    ...
    #[test]
    fn has_line_of_sight_needs_walkable_samples() {
        let open = open_mask();
        ...
        // Degenerate ray is always visible.
        assert!(has_line_of_sight(glam::Vec2::ZERO, glam::Vec2::ZERO, &closed));
    }

    #[test]
    fn enemy_ai_walks_toward_player() {
        let mut world = World::new();
        base_resources(&mut world);
        ...
        {
            let mut cmds = world.commands();
            player = spawn_player(&mut cmds, glam::Vec2::new(200.0, 0.0));
            enemy = spawn_enemy_at(&mut cmds, &catalog, EnemyKind::Maggot, glam::Vec2::ZERO, 1.0, false, false, 0);
        }
        world.flush();
        let _ = player;
        // Preset a walk burst with a disarmed attack timer so the only
        // active law is the deterministic walk-toward-player impulse.
        world.get_mut::<EnemyBrain>(enemy).unwrap().walk = 30.0;
        world.get_mut::<EnemyBrain>(enemy).unwrap().attack =
            GTimer::from_seconds(10.0, TimerMode::Once);
        let before = world.get::<Pos>(enemy).unwrap().0.distance(glam::Vec2::new(200.0, 0.0));
        schedule.run(&mut world);
        let after_pos = world.get::<Pos>(enemy).unwrap().0;
        let after = after_pos.distance(glam::Vec2::new(200.0, 0.0));
        assert!(after_pos.x > 0.0, "moved toward the player, got {after_pos:?}");
        assert!(after < before, "closed distance {before} -> {after}");
    }

    #[test]
    fn enemy_ai_close_bandit_backs_off_to_keep_range() {
        let mut world = World::new();
        base_resources(&mut world);
        let mut schedule = Schedule::default();
        schedule.add_systems(enemy_ai);
        ...
        {
            let mut cmds = world.commands();
            spawn_player(&mut cmds, glam::Vec2::ZERO);
        }
        world.flush();
        let enemy;
        {
        ...
        world.flush();
        // Arm the attack to fire this tick: dist < 48 takes the
        // unconditional repel branch (no RNG gate).
        world.get_mut::<EnemyBrain>(enemy).unwrap().attack =
            GTimer::from_seconds(DT, TimerMode::Once);
        schedule.run(&mut world);
        let vel = world.get::<Velocity>(enemy).unwrap().0;
        assert!(vel.x > 5.0, "backs away from the player, got {vel:?}");
        assert!(world.get::<EnemyBrain>(enemy).unwrap().walk > 0.0);
    }

    #[test]
    fn enemy_ai_bandit_fires_within_shoot_range() {
        let mut world = World::new();
        base_resources(&mut world);
        let mut schedule = Schedule::default();
        schedule.add_systems(enemy_ai);
        ...
        world.flush();
        // Each tick is an independent ~25% fire chance (LOS clear,
        // dist > 48); resetting state between ticks keeps them so.
        let mut fired = false;
        for _ in 0..100 {
            {
                let mut brain = world.get_mut::<EnemyBrain>(enemy).unwrap();
                brain.attack = GTimer::from_seconds(DT, TimerMode::Once);
                brain.walk = 0.0;
            }
            *world.get_mut::<Pos>(enemy).unwrap() = Pos(glam::Vec2::new(400.0, 0.0));
            *world.get_mut::<Velocity>(enemy).unwrap() = Velocity(glam::Vec2::ZERO);
            schedule.run(&mut world);
            let n = world
                .query::<(&Projectile, &Team)>()
                .iter(&world)
        ...
        assert!(fired, "bandit fired within 100 armed ticks");
        // Fired rounds carry the deflectable contact fade.
        let mut found_typ = false;
        for (_, _, typ) in world.query::<(&Projectile, &Team, &ProjectileTyp)>().iter(&world) {
            if typ.0 == 1 {
                found_typ = true;
            }
        }
        assert!(found_typ, "enemy bullet carries typ 1");
    }

    #[test]
    fn flush_pending_enemy_spawns_materializes() {
        let mut world = World::new();
        world.insert_resource(empty_catalog());
        let mut schedule = Schedule::default();
        schedule.add_systems(flush_pending_enemy_spawns);
        let pending;
        {
            let mut cmds = world.commands();
                    pending = cmds
        ...
        world.flush();
        schedule.run(&mut world);
        assert!(world.get_entity(pending).is_err(), "request consumed");
        assert_eq!(
            world.query::<&Enemy>().iter(&world).count(),
            1,
            "enemy spawned"
        );
        // GML loop law: Maggot base 2 hp at loops=2 → ceil(2*1.1) = 3.
        let hp = world.query::<&Health>().iter(&world).next().unwrap().hp;
        assert_eq!(hp, 3);
    }

    #[test]
    fn delayed_boss_gates_on_kills_then_flushes() {
        let mut world = World::new();
        world.insert_resource(empty_catalog());
        world.insert_resource(Run::default());
        world.insert_resource(open_mask());
        ...
        let mut schedule = Schedule::default();
        schedule.add_systems(tick_delayed_boss_spawns);
        let catalog = empty_catalog();
        let mut trash = Vec::new();
        {
            let mut cmds = world.commands();
            spawn_player(&mut cmds, glam::Vec2::ZERO);
            cmds.spawn(crate::comps_b::PendingDelayedBoss {
                kind: EnemyKind::Bandit,
                initial_trash: 5,
                kill_fraction: 0.6,
                from_wall: false,
            ...
            for i in 0..5 {
                trash.push(spawn_enemy_at(
                    &mut cmds,
                    &catalog,
                    EnemyKind::Maggot,
                    glam::Vec2::new(100.0 + i as f32 * 20.0, 0.0),
                    1.0,
                    false,
                    false,
                    0,
        ...
        world.flush();
        // 5 living trash: 0 kills < ceil(5*0.6)=3, gated.
        schedule.run(&mut world);
        assert_eq!(world.query::<&BossIntro>().iter(&world).count(), 0);
        for e in trash {
            world.despawn(e);
        }
        schedule.run(&mut world);
        assert_eq!(
            world.query::<&Enemy>().iter(&world).count(),
            1,
            "boss spawned after kills"
        );
        assert_eq!(world.query::<&BossIntro>().iter(&world).count(), 1);
        assert_eq!(world.resource::<Toast>().text, "BIG BANDIT");
    }

    #[test]
    fn frog_eggs_hatch_into_ballguy_and_acid_ring() {
        let mut world = World::new();
        world.insert_resource(sim_time());
        world.init_resource::<Run>();
        let mut schedule = Schedule::default();
        schedule.add_systems(tick_frog_eggs);
        let catalog = empty_catalog();
        let egg;
        {
            let mut cmds = world.commands();
        ...
        world.flush();
        schedule.run(&mut world);
        world.get_mut::<EnemyBrain>(egg).unwrap().attack =
            GTimer::from_seconds(DT, TimerMode::Once);
        schedule.run(&mut world);
        assert!(world.get_entity(egg).is_err(), "egg despawned");
        let pendings: Vec<_> = world.query::<&PendingEnemySpawn>().iter(&world).collect();
        assert_eq!(pendings.len(), 1);
        assert_eq!(pendings[0].kind, EnemyKind::Ballguy);
        let acids = world
            .query::<(&Projectile, &Team)>()
            .iter(&world)
            .filter(|(_, t)| **t == Team::Enemy)
            .count();
        assert_eq!(acids, 8, "8-way acid ring");
    }

    #[test]
    fn boss_intro_timing_holds_then_expires() {
        let mut world = World::new();
        world.insert_resource(sim_time());
        let mut schedule = Schedule::default();
        schedule.add_systems(tick_boss_intro);
        {
            world.commands().spawn(BossIntro {
                timer: GTimer::from_seconds(1.1, TimerMode::Once),
            });
        }
        schedule.run(&mut world);
    ...
    #[test]
    fn bigmaggot_charges_on_sight_and_inspector_pulls_player() {
        let mut world = World::new();
        world.insert_resource(sim_time());
        world.insert_resource(open_mask());
        let mut schedule = Schedule::default();
        schedule.add_systems(tick_bigmaggot_inspector);
        let catalog = empty_catalog();
        let (mag, insp);
        {
            let mut cmds = world.commands();
        ...
        schedule.run(&mut world);
        let _ = insp;
        // BigMaggot sees the player: charges at 90 px/s straight at them.
        let vel = world.get::<Velocity>(mag).unwrap().0;
        assert!(
            (vel - glam::Vec2::new(-90.0, 0.0)).length() < 1e-4,
            "bigmaggot charges, got {vel:?}"
        );
        // Inspector pull builds after 0.5 s of contact.
        for _ in 0..20 {
            schedule.run(&mut world);
        }
        let pv = world
            ...
            .0;
        assert!(pv.x > 0.5, "player dragged toward inspector, got {pv:?}");
    }

    #[test]
    fn explosive_kind_marks_jock_only() {
        assert!(explosive_kind(EnemyKind::Jock));
        assert!(!explosive_kind(EnemyKind::Bandit));
        assert!(!explosive_kind(EnemyKind::Scorpion));
    }

    #[test]
    fn random_spawn_pos_respects_min_distance() {
        use rand::SeedableRng;
        let mut rng = rand::rngs::SmallRng::seed_from_u64(7);
        let p = random_spawn_pos(&mut rng, 0.0);
        assert!(p.x.abs() <= ARENA_W / 2.0 - 80.0 + 1e-4);
        assert!(p.y.abs() <= ARENA_H / 2.0 - 80.0 + 1e-4);
        // Impossible demand yields the corner fallback.
        let fallback = random_spawn_pos(&mut rng, 1e9);
        assert_eq!(fallback, glam::Vec2::new(ARENA_W / 2.0 - 80.0, 0.0));
    }

    #[test]
    fn hit_warnings_and_shield_followers_tick() {
        let mut world = World::new();
        world.insert_resource(sim_time());
        let catalog = empty_catalog();
        let owner;
        {
            ...
            spawn_hit_warning(&mut cmds, glam::Vec2::ZERO);
        }
        world.flush();
        let mut schedule = Schedule::default();
        schedule.add_systems((tick_hit_warnings, tick_shield_followers));
        schedule.run(&mut world);
        assert_eq!(world.query::<&HitWarning>().iter(&world).count(), 1);
        // Follower tracks the owner's gunangle offset.
        world.get_mut::<EnemyBrain>(owner).unwrap().gunangle = 0.0;
        {
            world.commands().spawn((
                GameCleanup,
                LevelCleanup,
                ShieldFollower { owner },
                Pos(glam::Vec2::ZERO),
            ));
        }
        world.flush();
        ...
        let shield_pos = world
            .query::<(&ShieldFollower, &Pos)>()
            .iter(&world)
            .next()
            .unwrap()
            .1
            .0;
        assert_eq!(shield_pos, glam::Vec2::new(40.0 + 16.0, 0.0));
        // Warnings lapse after 0.5 s; followers die with their owner.
        for _ in 0..20 {
            schedule.run(&mut world);
        }
        assert_eq!(world.query::<&HitWarning>().iter(&world).count(), 0);
        world.despawn(owner);
        schedule.run(&mut world);
        // Owner despawn needs a flush before the follower notices.
        schedule.run(&mut world);
        assert_eq!(
            world
                .query::<(&ShieldFollower, &Pos)>()
    ...
    #[test]
    fn lil_hunter_die_spawns_trap_ring_and_skitters() {
        let mut world = World::new();
        base_resources(&mut world);
        // GML `LilHunterDie/Create_0`: speed 2 toward the player.
        let mut spawn_sched = Schedule::default();
        spawn_sched.add_systems(
            |mut commands: Commands, mut cues: ResMut<Queue<AudioCue>>| {
                spawn_lil_hunter_die(
                    &mut commands,
                    &mut cues,
                    glam::Vec2::ZERO,
                    Team::Enemy,
                    None,
                    Some(glam::Vec2::new(100.0, 0.0)),
                );
            },
        );
        spawn_sched.run(&mut world);
        let mut q = world.query::<(Entity, &Velocity, &LilHunterDie, &FxAngle)>();
        let (die, vel, trn, bounces, ang) = q
            .iter(&world)
            .next()
            .map(|(e, v, d, a)| (e, v.0, d.trn, d.bounces, a.0))
            .expect("die spawned");
        assert!((vel.length() - 60.0).abs() < 1e-3, "speed 2 px/step, got {vel:?}");
        assert!(vel.x > 59.0, "toward the player, got {vel:?}");
        assert!((5.0..=10.0).contains(&trn.abs()), "trn range, got {trn}");
        assert_eq!(bounces, 0);
        assert_eq!(ang, vel.y.atan2(vel.x).to_degrees() - 90.0);
        let cues = world.resource_mut::<Queue<AudioCue>>().drain();
        assert!(cues.iter().any(|c| c.name == "sndLilHunterBreak"));
        assert_eq!(world.query::<&PortalClear>().iter(&world).count(), 1);
        // GML `Step_0`: speed<6 → +2 (+0.05), direction += trn.
        let mut sched = Schedule::default();
        sched.add_systems(tick_lil_hunter_die);
        sched.run(&mut world);
        let vel = world.get::<Velocity>(die).unwrap().0;
        assert!((vel.length() / 30.0 - 4.05).abs() < 1e-3, "accel law, got {vel:?}");
        // GML death ring: 80 `TrapFire` at 2..2.2 px/step, team kept.
        let mut ring_sched = Schedule::default();
        ring_sched.add_systems(|mut commands: Commands| {
            spawn_lil_hunter_trapfire(&mut commands, glam::Vec2::ZERO, Team::Enemy);
        });
        ring_sched.run(&mut world);
        let mut q = world.query::<(&TrapFire, &HazardCloud, &Velocity, &Team)>();
        let ring: Vec<_> = q
            .iter(&world)
            .map(|(_, c, v, t)| (c.kind, v.0.length(), *t))
            .collect();
        assert_eq!(ring.len(), 80);
        for (kind, speed, team) in &ring {
            assert_eq!(*kind, HazardKind::Fire);
            assert!((60.0..=67.0).contains(speed), "fire speed, got {speed:?}");
