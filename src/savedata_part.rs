...
mod tests {
    ...
    #[test]
    fn healthcol_defaults_to_gml_red_and_parses_hex() {
        // GML `scrOptionsUpdate:96-100`: player color or (252, 56, 0).
        assert_eq!(
            SettingsData::default().healthcol_rgba(),
            [252.0 / 255.0, 56.0 / 255.0, 0.0, 1.0]
        );
        let mut s = SettingsData::default();
        s.player_color_hex = "#00ff00".to_string();
        assert_eq!(s.healthcol_rgba(), [0.0, 1.0, 0.0, 1.0]);
        s.player_color_hex = "bogus".to_string();
        assert_eq!(
            s.healthcol_rgba(),
            [252.0 / 255.0, 56.0 / 255.0, 0.0, 1.0]
        );
    ...
    #[test]
    fn achievement_registry_covers_all_59() {
        for id in 0..=58u8 {
            let (name, text, _, ty) = achievement_def(id).expect("entry exists");
            assert!(!name.is_empty() && !text.is_empty());
            assert!(ty <= 2);
            assert_eq!(name, name.to_ascii_uppercase());
        }
        assert!(achievement_def(59).is_none());
        assert!(!unlock_achievement(&mut SaveData::default(), 59));
    }

    #[test]
    fn achievement_unlock_dedupes_and_completes_set() {
        let mut save = SaveData::default();
        assert!(unlock_achievement(&mut save, 31));
        assert!(!unlock_achievement(&mut save, 31), "duplicate no-op");
        for id in 0..=58u8 {
            unlock_achievement(&mut save, id);
        }
        assert!(
            save.achievements.get("43").copied().unwrap_or(false),
            "ULTRA MUTANT auto-awards on completion"
        );
    }

    #[test]
    fn achievement_ids_follow_gml_tables() {
        use crate::data::EnemyKind;
        assert_eq!(achievement_for_race(RaceId::Melting), Some(0));
        assert_eq!(achievement_for_race(RaceId::Cuz), Some(44));
        assert_eq!(achievement_for_race(RaceId::Fish), None);
        assert_eq!(achievement_for_skin(RaceId::Steroids, 1), Some(16));
        assert_eq!(achievement_for_skin(RaceId::Steroids, 2), Some(52));
        assert_eq!(achievement_for_skin(RaceId::Fish, 0), None);
        assert_eq!(achievement_for_boss(EnemyKind::ThroneII), Some(35));
        assert_eq!(achievement_for_boss(EnemyKind::YvBoss), Some(51));
        assert_eq!(achievement_for_boss(EnemyKind::Captain), Some(42));
        assert_eq!(achievement_for_boss(EnemyKind::Bandit), None);
    }

    #[test]
    fn throne_defeat_skins_follow_gml_gates() {
        use crate::data::{MutationId, SkinLetter};
        let mut save = SaveData::default();
        for r in [RaceId::Melting, RaceId::Plant, RaceId::Eyes, RaceId::Steroids] {
            assert!(try_unlock_race(&mut save, r));
        }
        // Melting-B: no Rhino Skin / Strong Spirit.
        assert_eq!(
            throne_defeat_skins(&mut save, RaceId::Melting, &[], 99999, 50, 3),
            vec![(RaceId::Melting, SkinLetter::B)]
        );
        assert!(
            throne_defeat_skins(
                &mut save,
            ...
            .is_empty(),
            "rhino skin blocks Melting-B (already unlocked anyway)"
        );
        // Plant-B: sub-10-minute run.
        assert_eq!(
            throne_defeat_skins(&mut save, RaceId::Plant, &[], 17999, 50, 3),
            vec![(RaceId::Plant, SkinLetter::B)]
        );
        assert!(
            throne_defeat_skins(&mut save, RaceId::Plant, &[], 18000, 50, 3).is_empty()
        );
        // Eyes-C: pacifist.
        assert_eq!(
            throne_defeat_skins(&mut save, RaceId::Eyes, &[], 99999, 0, 3),
            vec![(RaceId::Eyes, SkinLetter::C)]
        );
        assert!(
            throne_defeat_skins(&mut save, RaceId::Eyes, &[], 99999, 1, 3).is_empty()
        );
        // Steroids-C: no gun picked up.
        assert_eq!(
            throne_defeat_skins(&mut save, RaceId::Steroids, &[], 99999, 50, 0),
            vec![(RaceId::Steroids, SkinLetter::C)]
        );
        assert!(
            throne_defeat_skins(&mut save, RaceId::Steroids, &[], 99999, 50, 1).is_empty()
        );
    }

    #[test]
    fn save_round_trip_preserves_unlocks_settings_crowns() {
        let mut save = SaveData::default();
        assert!(try_unlock_race(&mut save, RaceId::Crystal));
        save.unlock_crown(RaceId::Crystal, crown_port_to_gml(3));
        save.race_loadout_mut(RaceId::Fish).preferred_skin = 2;
        save.race_loadout_mut(RaceId::Fish).unlocked_skins[2] = true;
        save.total_runs = 7;
        save.high_score = 1234;
        save.settings.master_volume = 0.5;
        save.settings.profile_name = "tester".to_string();

        let text = serialize_save(&save).expect("serialize");
        let back = parse_save(&text).expect("parse");

        assert_eq!(back.version, SAVE_VERSION);
        assert_eq!(back.high_score, 1234);
        assert_eq!(back.total_runs, 7);
        assert!(back.unlocked_characters.iter().any(|s| s == "Crystal"));
        assert!(back.crown_unlocked(RaceId::Crystal, crown_port_to_gml(3)));
        assert!(back.any_crown_unlocked(RaceId::Crystal));
        assert_eq!(back.race_loadout(RaceId::Fish).preferred_skin, 2);
        assert!(back.skin_unlocked(RaceId::Fish, 2));
        assert_eq!(back.settings.master_volume, 0.5);
        assert_eq!(back.settings.profile_name, "tester");
        assert_eq!(back.settings.language, "en");
    }

    #[test]
    fn save_file_round_trip_through_disk() {
        let mut save = SaveData::default();
        save.unlock_crown(RaceId::Fish, crown_port_to_gml(2));
        let path = std::env::temp_dir().join(format!(
            "nt-rewrite-save-test-{}.json",
            std::process::id()
        ));
        store_save_to_file(&save, &path).expect("store");
        let back = load_save_from_file(&path).expect("load");
        let _ = std::fs::remove_file(&path);
        assert!(back.crown_unlocked(RaceId::Fish, crown_port_to_gml(2)));
        assert!(load_save_from_file(&path).is_err());
        assert_eq!(load_or_default(&path).high_score, 0);
    }

    #[test]
    fn race_loadout_gates_locked_crown_and_orphan_stored_gun() {
        let mut save = SaveData::default();
        // Locked crown pick is dropped.
        save.race_loadout_mut(RaceId::Fish).start_crown = 5;
        assert_eq!(save.race_loadout(RaceId::Fish).start_crown, 0);
        // Stored gun with no start gun is dropped.
        save.race_loadout_mut(RaceId::Fish).stored_weapon = WeaponId(5);
        assert_eq!(
            save.race_loadout(RaceId::Fish).stored_weapon,
            WeaponId(0)
        );
        // Unlocked crown pick survives.
        save.unlock_crown(RaceId::Fish, crown_port_to_gml(2));
        assert_eq!(save.race_loadout(RaceId::Fish).start_crown, 2);
    }

    #[test]
    fn sanitize_loadouts_resets_stale_picks() {
        let mut save = SaveData::default();
        let lo = save.race_loadout_mut(RaceId::Fish);
        lo.start_weapon = WeaponId(5);
        lo.stored_weapon = WeaponId(0);
        lo.start_crown = 9;
        save.sanitize_loadouts();
        let lo = save.race_loadout(RaceId::Fish);
        assert_eq!(lo.start_weapon, WeaponId(0));
        assert_eq!(lo.start_crown, 0);
    }

    #[test]
    fn crown_unlock_stamps_port_id_and_needs_race() {
        let mut save = SaveData::default();
        let gml = crown_port_to_gml(3);
        save.unlock_crown(RaceId::Rogue, gml);
        assert!(!save.crown_unlocked(RaceId::Rogue, gml));
        save.race_loadout_mut(RaceId::Rogue).unlocked = true;
        let haste_gml = crown_port_to_gml(3);
        ...
        assert_eq!(crown_gml_to_port(haste_gml), 3);
        assert_eq!(crown_gml_to_port(1), 0);
    }

    #[test]
    fn race_unlocked_falls_back_to_character_names() {
        let mut save = SaveData::default();
        // Legacy saves list names without race rows.
        save.unlocked_characters.push("Crystal".to_string());
    ...
    #[test]
    fn hardmode_chicken_b_needs_hardmode_sewers() {
        let mut save = SaveData::default();
        assert!(try_unlock_race(&mut save, RaceId::Chicken));
        // No hardmode: no B skin.
        check_area_skins(&mut save, AreaId::Sewers, 1, 0, RaceId::Chicken, false);
        assert!(!save.skin_unlocked(RaceId::Chicken, 1));
        // Hardmode sewers as Chicken: B skin.
        check_area_skins(&mut save, AreaId::Sewers, 1, 0, RaceId::Chicken, true);
        assert!(save.skin_unlocked(RaceId::Chicken, 1));
    }

    #[test]
    fn area_skin_thresholds() {
        let mut save = SaveData::default();
        // Sewers unlocks the Eyes race outright (GML area switch).
        check_area_skins(&mut save, AreaId::Sewers, 0, 0, RaceId::Fish, false);
        assert!(save.race_unlocked(RaceId::Eyes));
        // Cursed Caves unlocks Crystal B (race must be unlocked first).
        assert!(try_unlock_race(&mut save, RaceId::Crystal));
        check_area_skins(&mut save, AreaId::CursedCaves, 0, 0, RaceId::Fish, false);
        assert!(save.skin_unlocked(RaceId::Crystal, 1));
        // Area races unlock unconditionally: scrapyards→Plant,
        // mansion (port City)→Venuz, frozen city→Robot, jungle→Chicken,
        // labs→Steroids.
        check_area_skins(&mut save, AreaId::Scrapyards, 0, 0, RaceId::Fish, false);
        assert!(save.race_unlocked(RaceId::Plant));
        check_area_skins(&mut save, AreaId::City, 0, 0, RaceId::Fish, false);
        assert!(save.race_unlocked(RaceId::Venuz));
        check_area_skins(&mut save, AreaId::FrozenCity, 0, 0, RaceId::Fish, false);
        assert!(save.race_unlocked(RaceId::Robot));
        check_area_skins(&mut save, AreaId::Jungle, 0, 0, RaceId::Fish, false);
        assert!(save.race_unlocked(RaceId::Chicken));
        check_area_skins(&mut save, AreaId::Labs, 0, 0, RaceId::Fish, false);
        assert!(save.race_unlocked(RaceId::Steroids));
        // Desert needs loop 1+ for Rebel.
        check_area_skins(&mut save, AreaId::Desert, 0, 0, RaceId::Fish, false);
        assert!(!save.race_unlocked(RaceId::Rebel));
        check_area_skins(&mut save, AreaId::Desert, 1, 0, RaceId::Fish, false);
        assert!(save.race_unlocked(RaceId::Rebel));
        // HQ Horror/C needs ≤3 mutations held.
        assert!(try_unlock_race(&mut save, RaceId::Horror));
        check_area_skins(&mut save, AreaId::HQ, 0, 9, RaceId::Fish, false);
        assert!(!save.skin_unlocked(RaceId::Horror, 2));
        check_area_skins(&mut save, AreaId::HQ, 0, 3, RaceId::Fish, false);
        assert!(save.skin_unlocked(RaceId::Horror, 2));
    }

    #[test]
    fn robot_weapon_skin_names() {
        let mut save = SaveData::default();
        save.race_loadout_mut(RaceId::Robot).unlocked = true;
        check_robot_weapon_skins(&mut save, RaceId::Robot, "HYPER RIFLE");
        assert!(save.skin_unlocked(RaceId::Robot, 1));
        assert!(!save.skin_unlocked(RaceId::Robot, 2));
        check_robot_weapon_skins(&mut save, RaceId::Robot, "Rusty Revolver");
        assert!(save.skin_unlocked(RaceId::Robot, 2));
        check_robot_weapon_skins(&mut save, RaceId::Fish, "HYPER RIFLE");
    }

    #[test]
    fn crystal_damage_unlocks_skin_c_at_100() {
        let mut world = World::new();
        world.init_resource::<SaveData>();
        world.init_resource::<CrystalDamageTaken>();
        world
        ...
        world.spawn((
            ...
            Health {
                hp: 10,
                max: 10,
                invuln: crate::time::GTimer::disarmed(),
            },
        ));
        let mut sched = Schedule::default();
        sched.add_systems(tick_crystal_damage);
        sched.run(&mut world);
        world.query::<&mut Health>().single_mut(&mut world).unwrap().hp = 4;
        sched.run(&mut world);
        assert!(
            !world
                .resource::<SaveData>()
                .skin_unlocked(RaceId::Crystal, 2),
            "6 damage is not enough"
        );
        world.query::<&mut Health>().single_mut(&mut world).unwrap().hp = -90;
        sched.run(&mut world);
        assert!(
            world
                .resource::<SaveData>()
    ...
    #[test]
    fn global_skins_floor_and_run_count() {
        ...
        world
            .resource_mut::<SaveData>()
            .race_loadout_mut(RaceId::Melting)
            .unlocked = true;
        world
            .resource_mut::<SaveData>()
            .race_loadout_mut(RaceId::Plant)
            .unlocked = true;
        // Melting/C needs 12+ mutations; Plant/C needs 3+ blood sources
        // (blood crown + Bloodlust + blood hammer here).
        let mut player = Player::default();
        player.mutations = (0..12).map(|_| MutationId::Bloodlust).collect();
        player.crown = CrownKind::Blood;
        world.spawn((
            player,
            Inventory {
                // BLOOD HAMMER (id 67) is the third blood source here.
                weapons: [WeaponId(67), WeaponId::NONE, WeaponId::NONE],
        ...
        let mut sched = Schedule::default();
        sched.add_systems(tick_global_skins);
        sched.run(&mut world);
        let save = world.resource::<SaveData>();
        assert!(save.skin_unlocked(RaceId::Melting, 2));
        // Blood crown + Bloodlust + blood hammer = 3 blood sources.
        assert!(save.skin_unlocked(RaceId::Plant, 2));

        // Fish/B needs a loop as every non-hidden character.
        for r in crate::data::PLAYABLE_RACES {
            if matches!(r, RaceId::BigDog | RaceId::Skeleton | RaceId::Frog) {
                continue;
            }
            world
        ...
        sched.run(&mut world);
        assert!(world.resource::<SaveData>().skin_unlocked(RaceId::Fish, 1));
    }

    #[test]
    fn progress_unlocks_follow_floors() {
        let mut save = SaveData::default();
        let got = check_progress_unlocks(&mut save, 5, 0, false, false, false);
        assert!(got.contains(&RaceId::Crystal));
        assert!(got.contains(&RaceId::Eyes));
        assert!(!got.contains(&RaceId::Melting));
        assert!(is_race_unlocked(&save, RaceId::Crystal));
        assert!(!is_race_unlocked(&save, RaceId::Melting));
        let got = check_progress_unlocks(&mut save, 5, 0, true, false, false);
        assert!(got.contains(&RaceId::Melting));
        assert!(save.skin_unlocked(RaceId::Fish, 0));
        assert!(!save.skin_unlocked(RaceId::Crystal, 2));
    }
}
