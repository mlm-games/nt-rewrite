...
use crate::data::{AreaId, EnemyKind};
    ...
    fn permute_plan(chests: Vec<ChestSpawn>, ctx: ChestPermuteCtx) -> LevelPlan {
        let mut plan = generate_level(&run_for(1));
        plan.chests = chests;
        plan.enemies.clear();
        let _ = apply_chest_permutations(&mut plan, ctx);
        plan
    }

    fn base_ctx() -> ChestPermuteCtx {
        ChestPermuteCtx {
            area: AreaId::Desert,
            loops: 0,
            subarea: 1,
            rogue_in_run: false,
            player_half_health: false,
            crown_life: false,
            crown_love: false,
            open_mind: false,
            noradch: 0,
    ...
    #[test]
    fn chest_permutations_vault_keeps_proto_only() {
        let plan = permute_plan(
            vec![
                ChestSpawn::Weapon(glam::Vec2::new(10.0, 0.0)),
                ChestSpawn::Weapon(glam::Vec2::new(90.0, 0.0)),
                ChestSpawn::Ammo(glam::Vec2::ZERO),
                ChestSpawn::Rad(glam::Vec2::ZERO),
            ],
            ChestPermuteCtx {
                area: AreaId::Vault,
                ..base_ctx()
        ...
        assert_eq!(
            plan.chests,
            vec![ChestSpawn::Custom(
                ChestKind::Proto,
                glam::Vec2::new(90.0, 0.0)
            )],
            "furthest weapon becomes the ProtoChest, rest destroyed"
        );
    }

    #[test]
    fn chest_permutations_campfire_clears() {
        let plan = permute_plan(
            vec![
                ChestSpawn::Weapon(glam::Vec2::ZERO),
                ChestSpawn::Ammo(glam::Vec2::ZERO),
                ChestSpawn::Rad(glam::Vec2::ZERO),
            ],
            ChestPermuteCtx {
        ...
        assert!(plan.chests.is_empty());
    }

    #[test]
    fn game_hard_adds_loops_and_hardmode() {
        let mut run = run_for(1);
        assert_eq!(game_hard(&run), 1.0);
        run.loop_count = 1;
        assert_eq!(game_hard(&run), 17.0);
        run.hardmode = true;
        assert_eq!(game_hard(&run), 30.0);
    }

    #[test]
    fn chest_permutations_hardmode_desert_hands_big_weapon() {
        let plan = permute_plan(
            vec![
                ChestSpawn::Weapon(glam::Vec2::new(10.0, 0.0)),
                ChestSpawn::Ammo(glam::Vec2::new(20.0, 0.0)),
                ChestSpawn::Rad(glam::Vec2::ZERO),
            ],
            ChestPermuteCtx {
                hardmode: true,
        ...
        assert!(
            plan.chests.iter().any(|c| matches!(
                c,
                ChestSpawn::Custom(ChestKind::BigWeapon, _)
            )),
            "hardmode desert 1-1 adds a BigWeaponChest"
        );
        // Same floor without hardmode: none.
        let plan = permute_plan(
            vec![
                ChestSpawn::Weapon(glam::Vec2::new(10.0, 0.0)),
                ChestSpawn::Ammo(glam::Vec2::new(20.0, 0.0)),
        ...
        assert!(
            plan.chests.iter().all(|c| !matches!(
                c,
                ChestSpawn::Custom(ChestKind::BigWeapon, _)
            )),
            "no free big weapon off hardmode"
        );
    }

    #[test]
    fn chest_permutations_top_up_missing_kind_from_props() {
        // No weapon chest in plan, but props exist: the furthest
        // eligible prop past 160 px becomes one.
        let mut plan = generate_level(&run_for(1));
        plan.props.clear();
        plan.props.push((PropKind::Cactus, glam::Vec2::new(500.0, 0.0)));
        plan.props.push((PropKind::ThroneStatue, glam::Vec2::new(900.0, 0.0)));
        plan.chests = vec![
            ChestSpawn::Ammo(glam::Vec2::new(20.0, 0.0)),
            ChestSpawn::Rad(glam::Vec2::ZERO),
        ];
        plan.enemies.clear();
        let _ = apply_chest_permutations(&mut plan, base_ctx());
        assert!(
            plan.chests.iter().any(|c| matches!(
                c,
                ChestSpawn::Weapon(p) if *p == glam::Vec2::new(500.0, 0.0)
            )),
            "missing weapon tops up from the cactus, not the statue: {:?}",
            plan.chests
                .iter()
    ...
    #[test]
    fn chest_permutations_trims_to_one_per_kind() {
        let plan = permute_plan(
            vec![
                ChestSpawn::Weapon(glam::Vec2::new(1.0, 0.0)),
                ChestSpawn::Weapon(glam::Vec2::new(2.0, 0.0)),
                ChestSpawn::Weapon(glam::Vec2::new(3.0, 0.0)),
                ChestSpawn::Ammo(glam::Vec2::ZERO),
                ChestSpawn::Rad(glam::Vec2::ZERO),
            ],
            base_ctx(),
        ...
        let weapons = plan
            .chests
            .iter()
            .filter(|c| matches!(c, ChestSpawn::Weapon(_)))
            .count();
        assert_eq!(weapons, 1, "base trim keeps a single weapon chest");
    }

    #[test]
    fn chest_permutations_love_converts_to_ammo() {
        let plan = permute_plan(
            vec![
                ChestSpawn::Weapon(glam::Vec2::ZERO),
                ChestSpawn::Ammo(glam::Vec2::ZERO),
        ...
        assert!(
            plan.chests.iter().all(|c| matches!(
                c,
                ChestSpawn::Custom(ChestKind::Ammo, _)
            )),
            "love converts every chest to ammo: {:?}",
            plan.chests
                .iter()
                .map(|c| c.pos().to_string())
                .collect::<Vec<_>>()
    ...
    #[test]
    fn chest_permutations_rogue_converts_rad() {
        let plan = permute_plan(
            vec![
                ChestSpawn::Weapon(glam::Vec2::new(10.0, 0.0)),
                ChestSpawn::Ammo(glam::Vec2::new(20.0, 0.0)),
        ...
        assert!(plan.chests.contains(&ChestSpawn::Custom(
            ChestKind::Rogue,
            glam::Vec2::ZERO
        )));
        assert_eq!(
            plan.chests.iter().filter(|c| matches!(c, ChestSpawn::Rad(_))).count(),
            0,
            "no plain rad survives a Rogue run"
        );
    }

    #[test]
    fn chest_permutations_starved_rad_hatches_horror_once() {
        let ctx = ChestPermuteCtx {
            noradch: 2,
            ..base_ctx()
        };
        let mut plan = generate_level(&run_for(1));
        ...
        plan.enemies.clear();
        let out = apply_chest_permutations(&mut plan, ctx);
        assert!(out.horror, "first starved rad hatches the horror");
        assert_eq!(plan.enemies.len(), 1);
        assert_eq!(plan.enemies[0].0, EnemyKind::HostileHorror);
        assert!(
            plan.chests.iter().all(|c| !matches!(c, ChestSpawn::Rad(_))),
            "horror replaces the rad chest"
        );

        // A later starved chest degrades to RadBig instead.
        let mut plan = generate_level(&run_for(1));
        plan.chests = vec![
            ChestSpawn::Weapon(glam::Vec2::new(10.0, 0.0)),
            ChestSpawn::Ammo(glam::Vec2::new(20.0, 0.0)),
        ...
        plan.enemies.clear();
        let out = apply_chest_permutations(
            &mut plan,
            ChestPermuteCtx {
                noradch: 3,
                horror_done: true,
        ...
        assert!(
            plan.chests.iter().all(|c| !matches!(c, ChestSpawn::Rad(_))),
            "starved rad degrades past the first horror"
        );
    }
