...
mod tests {
    use super::*;
    use crate::comps_a::FloorMask;
    use crate::comps_a::Player as PlayerComp;
    use crate::comps_b::ThroneRoomState;
    ...
    fn test_brain() -> EnemyBrain {
        EnemyBrain {
            speed: 60.0,
            accel: 800.0,
            preferred_range: 100.0,
            shoot_range: 500.0,
            attack: GTimer::from_seconds(0.01, TimerMode::Repeating),
            burst_left: 0,
    ...
    fn spawn_boss(world: &mut World, kind: EnemyKind, pos: glam::Vec2) -> Entity {
        world
            .spawn((
                GameCleanup,
                LevelCleanup,
    ...
    fn spawn_player(world: &mut World, pos: glam::Vec2) {
        world.spawn((
            GameCleanup,
            LevelCleanup,
            PlayerComp::default(),
    ...
    fn boss_world(kind: EnemyKind) -> World {
        ...
        spawn_player(&mut world, glam::Vec2::new(150.0, 0.0));
        // Timers that fire almost every tick so every handler acts.
        let mut boss = BossBrain::new(kind, glam::Vec2::ZERO);
        boss.attack_timer = GTimer::from_seconds(0.01, TimerMode::Repeating);
        boss.special_timer = GTimer::from_seconds(0.01, TimerMode::Repeating);
    ...
    fn run_boss(world: &mut World, ticks: u32) {
        let mut schedule = Schedule::default();
        schedule.add_systems(boss_ai);
        for _ in 0..ticks {
            schedule.run(world);
        }
    }

    fn activity(world: &mut World) -> (usize, usize, usize, usize) {
        let projectiles = world.query::<&Projectile>().iter(world).count();
        let beams = world.query::<&Beam>().iter(world).count();
        let pending = world.query::<&PendingEnemySpawn>().iter(world).count();
        let crystals = world.query::<&HyperOrbitCrystal>().iter(world).count();
        (projectiles, beams, pending, crystals)
    }

    #[test]
    fn dispatcher_routes_every_boss_kind() {
        let kinds = [
            EnemyKind::BigBandit,
            EnemyKind::BigBanditLoop,
            EnemyKind::BigDog,
        ...
        for kind in kinds {
            let mut world = boss_world(kind);
            run_boss(&mut world, 6);
            let (projectiles, beams, pending, crystals) = activity(&mut world);
            let mut boss_q = world.query::<(&Enemy, &BossBrain, &EnemyBrain)>();
            let (_, boss, brain) = boss_q.single(&world).expect("boss present");
            assert_eq!(boss.phase, boss.phase, "query sanity");
            let acted = projectiles + beams + pending + crystals > 0
                || boss.pattern_index > 0
                || boss.phase != BossPhase::Idle
                || brain.walk > 0.0
                || brain.burst_left > 0
                || brain.ammo > 0;
            assert!(acted, "{kind:?} handler produced no observable action");
            let _ = boss;
        }
    }

    #[test]
    fn dispatcher_skips_non_boss_brains() {
        // A BossBrain on a non-boss kind must not tick (bevy `continue`
        // sits before the first timer tick).
        let mut world = World::new();
        world.insert_resource(sim_time());
        world.insert_resource(Run::default());
        world.insert_resource(Trauma::default());
        ...
        spawn_player(&mut world, glam::Vec2::new(150.0, 0.0));
        let mut boss = BossBrain::new(EnemyKind::Bandit, glam::Vec2::ZERO);
        boss.attack_timer = GTimer::from_seconds(0.01, TimerMode::Repeating);
        world.spawn((
            GameCleanup,
            LevelCleanup,
        ...
        run_boss(&mut world, 6);
        let (projectiles, beams, pending, crystals) = activity(&mut world);
        assert_eq!((projectiles, beams, pending, crystals), (0, 0, 0, 0));
        let mut q = world.query::<(&Enemy, &BossBrain)>();
        let (_, boss) = q.single(&world).expect("bandit present");
        assert_eq!(boss.pattern_index, 0);
        assert_eq!(boss.phase, BossPhase::Idle);
    }

    #[test]
    fn big_bandit_fan_fires_five_rounds() {
        let mut world = World::new();
        let mut schedule = Schedule::default();
        ...
        let count = world.query::<&Projectile>().iter(&world).count();
        assert_eq!(count, 5);
        for vel in world.query::<&Velocity>().iter(&world) {
            assert!((vel.0.length() - 240.0).abs() < 0.01);
        }
    }

    #[test]
    fn throne_triplet_volley_fires_six() {
        // GML `Alarm_2` with a loaded volley: exactly the mirrored
        // Horror triplets (2 sides x 3 offsets).
        let mut world = boss_world(EnemyKind::Throne);
        {
            let mut bosses = world.query::<(&Enemy, &mut BossBrain, &mut EnemyBrain)>();
            let (_, mut boss, mut brain) = bosses.single_mut(&mut world).expect("boss");
            boss.pattern_index = 1;
            brain.ammo = 3;
            brain.gunangle = 0.0;
        }
        run_boss(&mut world, 3);
        let count = world.query::<&Projectile>().iter(&world).count();
        assert_eq!(count, 6, "throne volley is 6 mirrored bullets");
    }

    #[test]
    fn hyper_ensure_orbit_scales_and_caps() {
        for (loop_count, enraged, want) in [(0, false, 3), (1, false, 5), (2, false, 7), (3, true, 10)]
        {
            let mut world = World::new();
            world.init_resource::<Run>();
            let mut schedule = Schedule::default();
            schedule.add_systems(
                move |mut commands: Commands, mut run: ResMut<Run>| {
                    hyper_ensure_orbit(
                        &mut commands,
                        Entity::PLACEHOLDER,
                        glam::Vec2::ZERO,
            ...
            schedule.run(&mut world);
            let count = world.query::<&HyperOrbitCrystal>().iter(&world).count();
            assert_eq!(count, want, "loop={loop_count} enraged={enraged}");
            // GML: real LaserCrystal enemies, loop-scaled hp, kill
            // count decremented per crystal.
            assert_eq!(
                world.query::<&Enemy>().iter(&world).count(),
                want
            );
            let hp = world.query::<&Health>().iter(&world).next().unwrap().hp;
            let expect = crate::enemies::spawn_hp(
                EnemyKind::LaserCrystal,
                enemy_def(EnemyKind::LaserCrystal).hp,
                loop_count,
            );
            assert_eq!(hp, expect);
        }
        // Cap at 12 for deep loops.
        let mut world = World::new();
        world.init_resource::<Run>();
        let mut schedule = Schedule::default();
        schedule.add_systems(|mut commands: Commands, mut run: ResMut<Run>| {
            hyper_ensure_orbit(&mut commands, Entity::PLACEHOLDER, glam::Vec2::ZERO, 10, true, &mut run);
        });
        schedule.run(&mut world);
        assert_eq!(world.query::<&HyperOrbitCrystal>().iter(&world).count(), 12);
    }

    #[test]
    fn hyper_crystals_tick_and_fire() {
        let mut world = World::new();
        world.insert_resource(sim_time());
        let owner = spawn_boss(&mut world, EnemyKind::Hyper, glam::Vec2::ZERO);
        let crystal = world
            .spawn((
                GameCleanup,
                LevelCleanup,
                ...
                Hitbox { radius: 9.0 },
                Velocity(glam::Vec2::ZERO),
                Pos(glam::Vec2::new(70.0, 0.0)),
                HyperOrbitCrystal {
                    owner,
                    angle: 0.0,
                    radius: 70.0,
                    angular_speed: 1.15,
                    fire_timer: GTimer::from_seconds(0.01, TimerMode::Repeating),
                },
            ))
            .id();
        let mut schedule = Schedule::default();
        schedule.add_systems(tick_hyper_orbit_crystals);
        schedule.run(&mut world);
        // Snapped onto the orbit and fired a beam.
        let pos = world.get::<Pos>(crystal).expect("crystal alive").0;
        let want = orbit_point(glam::Vec2::ZERO, 70.0, 1.15 * DT);
        assert!((pos - want).length() < 0.01, "pos={pos} want={want}");
        assert_eq!(world.query::<&Beam>().iter(&world).count(), 1);

        // Orphaned crystals drift down and pop.
        world.despawn(owner);
        {
            let mut vel = world.get_mut::<Velocity>(crystal).expect("crystal alive");
            vel.0 = glam::Vec2::ZERO;
        }
        for _ in 0..4 {
            schedule.run(&mut world);
        }
        assert!(world.get::<Pos>(crystal).is_none(), "orphan despawns");
    }

    #[test]
    fn difficulty_for_loop_scales() {
        assert_eq!(difficulty_for_loop(false), 1.0);
        assert_eq!(difficulty_for_loop(true), 1.25);
    }

    #[test]
    fn lil_hunter_fire_ring_spawns_eighty_clouds() {
        let mut world = World::new();
        let mut schedule = Schedule::default();
        schedule.add_systems(move |mut commands: Commands| {
            lil_hunter_fire_ring(&mut commands, glam::Vec2::ZERO);
        });
        schedule.run(&mut world);
        let count = world.query::<&HazardCloud>().iter(&world).count();
        assert_eq!(count, 80, "GML landing/death ring is 80 flames");
    }

    #[test]
    fn throne_beam_charges_then_fires() {
        // A Throne mid-charge (`Telegraph`) emits the downward
        // `NothingBeam` when the 30-tick charge completes.
        let mut world = boss_world(EnemyKind::Throne);
        {
            let mut bosses = world.query::<(&Enemy, &mut BossBrain)>();
            let (_, mut boss) = bosses.single_mut(&mut world).expect("boss");
            boss.set_phase(BossPhase::Telegraph, 0.05);
            boss.attack_timer = GTimer::from_seconds(100.0, TimerMode::Once);
            boss.special_timer = GTimer::from_seconds(100.0, TimerMode::Once);
        }
        run_boss(&mut world, 5);
        let beams: Vec<Beam> = world.query::<&Beam>().iter(&world).cloned().collect();
        assert_eq!(beams.len(), 1, "charged throne beam fires once");
        assert_eq!(beams[0].dir, glam::Vec2::new(0.0, 1.0));
        assert_eq!(beams[0].team, Team::Enemy);
    }

    #[test]
    fn pattern_helpers_match_bevy_laws() {
        assert!(fan_angles(0.0, 0, 0.1).is_empty());
        assert_eq!(fan_angles(2.0, 1, 0.5), vec![2.0]);
        let fan = fan_angles(1.0, 5, 0.25);
        assert!((fan[2] - 1.0).abs() < 1e-4);
        assert_eq!(ring_angles(4, 0.0).len(), 4);
        assert!((ring_angles(4, 0.0)[1] - std::f32::consts::FRAC_PI_2).abs() < 1e-4);
        assert_eq!(hyper_orbit_count(0), 3);
        assert_eq!(hyper_orbit_count(1), 5);
        assert_eq!(hyper_orbit_count(2), 7);
        let p = orbit_point(glam::Vec2::new(2.0, 3.0), 10.0, 0.0);
        assert!((p.x - 12.0).abs() < 1e-4);
        let d = lead_target(glam::Vec2::ZERO, glam::Vec2::ZERO, glam::Vec2::ZERO, 0.0);
        assert!(d.is_finite());
        assert_eq!(split_line_dirs(0.0, 8).len(), 8);
        assert_eq!(star_angles(12, 0.1).len(), 12);
    }

    #[test]
    fn boss_taunt_fires_once_past_delay() {
        // GML `BanditBoss/Other_10:25-30`: no player → delay ticks past
        // 50 → `sndBigBanditTaunt` once.
        let mut world = World::new();
        world.insert_resource(Queue::<AudioCue>::default());
        let e = spawn_boss(&mut world, EnemyKind::BigBandit, glam::Vec2::ZERO);
        let mut sched = Schedule::default();
        sched.add_systems(tick_boss_taunts);
        for _ in 0..50 {
            sched.run(&mut world);
        }
        ...
        assert!(cues.iter().any(|c| c.name == "sndBigBanditTaunt"));
        assert!(world.get::<BossBrain>(e).unwrap().taunt);
        // Latch: never fires twice.
        sched.run(&mut world);
        assert!(world.resource::<Queue<AudioCue>>().is_empty());
    }

    #[test]
    fn throne_taunt_needs_longer_silence_and_yv_resets() {
        let mut world = World::new();
        world.insert_resource(Queue::<AudioCue>::default());
        spawn_boss(&mut world, EnemyKind::Throne, glam::Vec2::ZERO);
        spawn_boss(&mut world, EnemyKind::ThroneII, glam::Vec2::ZERO);
        let yv = spawn_boss(&mut world, EnemyKind::YvBoss, glam::Vec2::ZERO);
        let mut sched = Schedule::default();
        sched.add_systems(tick_boss_taunts);
        for _ in 0..60 {
            sched.run(&mut world);
        }
        // YV + Throne-II already taunted at 50 (`Nothing2/Other_10:21`),
        // the Throne stays silent past 50.
        let cues = world.resource_mut::<Queue<AudioCue>>().drain();
        assert!(cues.iter().any(|c| c.name == "sndGunGodTaunt"));
        assert!(cues.iter().any(|c| c.name == "sndNothing2Taunt"));
        assert!(!cues.iter().any(|c| c.name == "sndNothingTaunt"));
        // GML `YVBoss/Step_0:27`: a live player resets YV's delay.
        spawn_player(&mut world, glam::Vec2::new(150.0, 0.0));
        sched.run(&mut world);
        assert_eq!(world.get::<BossBrain>(yv).unwrap().tauntdelay, 0);
        assert!(world.resource::<Queue<AudioCue>>().is_empty());
        // Back to silence: the Throne needs >150 (GML `Nothing` gate).
        let players: Vec<Entity> =
            world.query_filtered::<Entity, With<Player>>().iter(&world).collect();
        for p in players {
            world.despawn(p);
        }
        for _ in 0..90 {
