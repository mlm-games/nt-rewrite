...
mod tests {
    ...
    fn base_health(hp: i32, max: i32) -> Health {
        Health {
            hp,
            max,
    ...
    fn spawn_player_full(world: &mut World, player: Player, hp: i32, max: i32) -> Entity {
        world
            .spawn((
                player,
                base_health(hp, max),
    ...
    #[test]
    fn portal_enter_robot_eats_weapons_with_auto_collect() {
        // GML `Portal/Collision_Player:20-24`: Robot eats nearby loose
        // guns (`scrRobotEat(other.wep, true)`); cursed guns are skipped.
        let mut world = portal_world();
        let e = spawn_player_full(&mut world, Player::default(), 8, 8);
        world.get_mut::<RaceState>(e).unwrap().race = RaceId::Robot;
        world.spawn((
            Portal,
            PortalState {
                kind: 1,
        ...
        let meal = world
            .spawn((
                Pickup {
                    kind: PickupKind::Weapon(WeaponId(39)),
                },
        ...
        let cursed = world
            .spawn((
                ...
                PickupCurse,
                Pos(glam::Vec2::new(12.0, 0.0)),
            ))
            .id();
        let mut sched = Schedule::default();
        sched.add_systems(portal_enter);
        sched.run(&mut world);
        assert!(world.get_entity(meal).is_err(), "eaten gun is gone");
        assert!(
            world.get_entity(cursed).is_ok(),
            "cursed guns are skipped"
        );
        let ammo_total: i32 = world.get::<Inventory>(e).unwrap().ammo.iter().sum();
        assert!(ammo_total > 0, "auto-collected drops grant ammo");
    }

    // -- roll_mutations determinism --------------------------------------

    #[test]
    fn roll_mutations_seeded_is_deterministic() {
        let mk = || {
        ...
        let ra = roll_mutations_with(&mut a, &mut rng_a);
        let rb = roll_mutations_with(&mut b, &mut rng_b);
        assert_eq!(ra.len(), 4);
        assert_eq!(ra, rb);
        // No duplicates, none owned.
    ...
    #[test]
    fn roll_mutations_skips_owned_and_patience_when_used() {
        let mut p = Player::default();
        p.mutations = ALL_MUTATIONS.to_vec();
        p.mutations.retain(|m| *m != MutationId::Patience);
        p.patience_used = true;
        // Only Patience is unowned but excluded: empty pool.
        assert!(roll_mutations(&mut p).is_empty());

        let mut p = Player::default();
        p.patience_used = true;
        let mut rng = rand::rngs::StdRng::seed_from_u64(7);
        for _ in 0..50 {
            let c = roll_mutations_with(&mut p, &mut rng);
            assert!(!c.contains(&MutationId::Patience));
        }
    }

    #[test]
    fn roll_mutations_destiny_rolls_one_and_patience_consumes() {
        let mut p = Player::default();
        p.crown = CrownKind::Destiny;
        assert_eq!(roll_mutations(&mut p).len(), 1);

        let mut p = Player::default();
        p.patience_bonus = true;
        assert_eq!(roll_mutations(&mut p).len(), 4);
        assert!(!p.patience_bonus);
    }

    #[test]
    fn ultra_choices_are_two_distinct_per_race() {
        for race in crate::data::PLAYABLE_RACES {
            let [a, b] = ultra_choices_for(race);
            assert_ne!(a, b);
        }
    }

    // -- apply_mutation stat effects --------------------------------------

    fn mutation_world() -> World {
        ...
        spawn_player_full(&mut world, Player::default(), 8, 8);
        world
    }

    fn apply_in_world(world: &mut World, id: MutationId) {
        let mut sched = Schedule::default();
        sched.add_systems(
            move |mut q: Query<
                (&mut Player, &mut Health, &mut Inventory, &RaceState),
                With<Player>,
            >,
             mut flags: MutationFlagSet,
             mut fx: LevelFx,
    ...
    fn player_map<T>(world: &mut World, f: impl FnOnce(&Player) -> T) -> T {
        let mut q = world.query::<&Player>();
        f(q.iter(world).next().expect("player"))
    }

    #[test]
    fn apply_mutation_strong_spirit_readies() {
        let mut world = mutation_world();
        apply_in_world(&mut world, MutationId::StrongSpirit);
        assert!(player_map(&mut world, |p| p.strong_spirit_ready));
        assert!(player_map(&mut world, |p| p
            .mutations
            .contains(&MutationId::StrongSpirit)));
        assert!(world.resource::<Toast>().text.contains("Strong Spirit"));
    }

    #[test]
    fn apply_mutation_scarier_face_sets_flag() {
        let mut world = mutation_world();
        apply_in_world(&mut world, MutationId::ScarierFace);
        assert!(world.resource::<ScarierFace>().0);
        assert!(player_map(&mut world, |p| p
            .mutations
            .contains(&MutationId::ScarierFace)));
    }

    #[test]
    fn apply_mutation_eagle_eyes_tightens_spread() {
        let mut world = mutation_world();
        apply_in_world(&mut world, MutationId::EagleEyes);
        assert!((player_map(&mut world, |p| p.spread_mult) - 0.4).abs() < 1e-6);
    }

    #[test]
    fn apply_mutation_rhino_skin_and_throne_butt() {
        let mut world = mutation_world();
        apply_in_world(&mut world, MutationId::RhinoSkin);
        let mut hq = world.query::<&Health>();
        let h = hq.iter(&world).next().unwrap();
        assert_eq!((h.hp, h.max), (12, 12));

        // Throne Butt on Crystal: +2 HP and ability mult.
        let mut world = mutation_world();
        {
            let mut rq = world.query::<&mut RaceState>();
            rq.iter_mut(&mut world).next().unwrap().race = RaceId::Crystal;
        }
        apply_in_world(&mut world, MutationId::ThroneButt);
        assert!(player_map(&mut world, |p| p.throne_butt));
        assert!((player_map(&mut world, |p| p.ultra_ability_mult) - 1.15).abs() < 1e-6);
        let mut hq = world.query::<&Health>();
        assert_eq!(hq.iter(&world).next().unwrap().max, 10);
    }

    // -- level ups --------------------------------------------------------

    fn levelup_world(rads: u32) -> World {
        let mut world = World::new();
        world.init_resource::<Trauma>();
        ...
        let mut p = Player::default();
        p.rads = rads;
        spawn_player_full(&mut world, p, 8, 8);
        world
    }

    fn run_check_level_up(world: &mut World) {
        let mut sched = Schedule::default();
        sched.add_systems(
            |mut commands: Commands,
             mut trauma: ResMut<Trauma>,
             mut flash: ResMut<FlashWhite>,
             mut pq: Query<(&mut Player, &Pos), With<Player>>,
             mut toast: ResMut<Toast>,
             audio: Res<GameAudio>,
             mut cues: ResMut<Queue<AudioCue>>| {
                let (mut p, pos) = pq.single_mut().unwrap();
                let at = pos.0;
                check_level_up(
                    &mut commands,
                    &mut trauma,
    ...
    #[test]
    fn level_up_feedback_gated_on_rads() {
        // Below threshold: silent.
        let mut world = levelup_world(59);
        run_check_level_up(&mut world);
        assert_eq!(player_map(&mut world, |p| p.level), 1);
        assert!(world.resource::<Toast>().text.is_empty());
        assert!(world.resource::<Queue<AudioCue>>().is_empty());

        // At threshold: LEVEL UP + pick owed + jingle + burst.
        let mut world = levelup_world(60);
        run_check_level_up(&mut world);
        assert_eq!(player_map(&mut world, |p| p.level), 2);
        assert_eq!(player_map(&mut world, |p| p.mutation_picks_owed), 1);
        assert_eq!(world.resource::<Toast>().text, "LEVEL UP!");
        let cues = world.resource_mut::<Queue<AudioCue>>().drain();
        assert_eq!(cues.len(), 1);
        assert_eq!(cues[0].name, "sndLevelUp");
    }

    #[test]
    fn level_ten_owes_ultra_pick() {
        let mut world = World::new();
        world.init_resource::<Trauma>();
        world.init_resource::<FlashWhite>();
        world.init_resource::<Toast>();
        ...
        let mut p = Player::default();
        p.level = 9;
        p.next_level_rads = 9 * 60;
        p.rads = 9 * 60;
        spawn_player_full(&mut world, p, 8, 8);
        run_check_level_up(&mut world);
        assert_eq!(player_map(&mut world, |p| p.level), 10);
        assert!(player_map(&mut world, |p| p.ultra_pick_owed));
        assert_eq!(world.resource::<Toast>().text, "LEVEL ULTRA!");
    }

    #[test]
    fn mutation_choice_applies_and_unpauses() {
        let mut world = World::new();
        ...
        let mut p = Player::default();
        p.mutation_picks_owed = 1;
        spawn_player_full(&mut world, p, 8, 8);
        world.insert_resource(PendingMutation {
            choices: vec![MutationId::EagleEyes, MutationId::RhinoSkin],
        });
        world.resource_mut::<MutationChoice>().0 = Some(0);
        let mut sched = Schedule::default();
        sched.add_systems(handle_mutation_choice);
        sched.run(&mut world);
        assert!(player_map(&mut world, |p| p
            .mutations
            .contains(&MutationId::EagleEyes)));
        assert_eq!(player_map(&mut world, |p| p.mutation_picks_owed), 0);
    ...
    #[test]
    fn portal_check_opens_when_area_clear() {
        let mut world = portal_world();
        spawn_player_full(&mut world, Player::default(), 8, 8);
        // Blocked while enemies remain.
        world.spawn((
            Enemy {
            ...
            base_health(4, 4),
            Pos(glam::Vec2::new(500.0, 500.0)),
        ));
        let mut sched = Schedule::default();
        sched.add_systems(portal_check);
        sched.run(&mut world);
        assert!(!world.resource::<Run>().portal_open);
        // Clear enemies (no mask cells -> pos falls back to origin).
        let mut eq = world.query_filtered::<Entity, With<Enemy>>();
        let foes: Vec<Entity> = eq.iter(&world).collect();
        for e in foes {
            world.despawn(e);
        }
        sched.run(&mut world);
        assert!(world.resource::<Run>().portal_open);
        assert_eq!(world.query::<&Portal>().iter(&world).count(), 1);
        assert_eq!(world.query::<&PortalShock>().iter(&world).count(), 1);
        assert_eq!(world.query::<&PortalClear>().iter(&world).count(), 1);
    }

    #[test]
    fn portal_spawn_waits_strip_then_shocks() {
        // Bevy `animate_portal`: Spawn gates on the spawn-strip oneshot
        // finishing (no strip → immediately Idle + second 72px shock).
        let mut world = portal_world();
        let portal = world
            .spawn((
                Portal,
            ...
            .id();
        let mut sched = Schedule::default();
        sched.add_systems(animate_portal);
        sched.run(&mut world);
        assert_eq!(
            world.get::<PortalState>(portal).unwrap().phase,
            PortalPhase::Idle,
            "no strip opens immediately"
        );
        assert!(
            world
                .query::<&PortalShock>()
                .iter(&world)
                .any(|s| (s.radius - 72.0).abs() < 1e-6),
            "second shock spawned"
        );
    }

    #[test]
    fn portal_spawn_strip_gate_holds_until_finished() {
        // With a live spawn-strip oneshot, Spawn holds until the strip
        // finishes, then flips Idle and swaps to the idle strip.
        let mut world = portal_world();
        let catalog = AnimCatalog::from_json(
            r#"{"images/sprPortalSpawn.png": {"frames": 2, "w": 32, "h": 32, "fps": 8.0, "xorigin": 16.0, "yorigin": 16.0},
                "images/sprPortal.png": {"frames": 4, "w": 32, "h": 32, "fps": 8.0, "xorigin": 16.0, "yorigin": 16.0}}"#,
            AtlasDesc {
                size: 256,
                max_pages: 1,
            padding: 0,
        ...
        let portal = world
            .spawn((
                ...
                Pos(glam::Vec2::ZERO),
                crate::anim::SpriteAnim::oneshot(
                    "images/sprPortalSpawn.png",
                    world
                        .resource::<AnimCatalog>()
                        .def("images/sprPortalSpawn.png")
                        .unwrap(),
                ),
            ))
            .id();
        // Advance the strip via the shared animator, then the machine.
        let mut anim_sched = Schedule::default();
        anim_sched.add_systems(crate::anim::animate_sprites);
        let mut sched = Schedule::default();
        sched.add_systems(animate_portal);
        sched.run(&mut world);
        assert_eq!(
        ...
        assert_eq!(
            world
                .get::<crate::anim::SpriteAnim>(portal)
                .unwrap()
                .path,
    ...
    #[test]
    fn portal_enter_kicks_transition_via_close_timer() {
        let mut world = portal_world();
        ...
        sched.add_systems(portal_enter);
        sched.run(&mut world);
        assert!(world.get::<PortalClosing>(portal).is_some());
        assert!(world.get::<PortalSucking>(portal).is_none());

        // Run the close timer out: animate_portal kicks the suck.
        let mut sched = Schedule::default();
        sched.add_systems(animate_portal);
        for _ in 0..100 {
            sched.run(&mut world);
        }
        let mut sq = world.query_filtered::<&PortalSucking, With<Player>>();
        assert_eq!(sq.iter(&world).count(), 1);
    }

    #[test]
    fn throne_sit_beats_into_win() {
        let mut world = portal_world();
        ...
        world.resource_mut::<crate::input::NtInput>().press_interact();
        let mut sched = Schedule::default();
        sched.add_systems(tick_throne_sit);
        sched.run(&mut world);
        sched.run(&mut world);
        assert_eq!(
            world.query::<&crate::comps_b::ThroneSit>().iter(&world).count(),
            1,
            "interact at the zone sits"
        );
        for _ in 0..400 {
            sched.run(&mut world);
        }
        let run = world.resource::<Run>();
        assert!(run.game_over && run.won, "345-tick beat ends won");
    }

    #[test]
    fn portal_suck_advances_floor_and_hides_player() {
        let mut world = portal_world();
        world.resource_mut::<Run>().portal_open = true;
        let portal = world
            .spawn((
                Portal,
                PortalState {
        ...
        world.spawn((
            ...
            Pos(glam::Vec2::new(5.0, 0.0)),
        ));
        let player = spawn_player_full(&mut world, Player::default(), 8, 8);
        world.entity_mut(player).insert(PortalSucking {
            portal,
            timer: GTimer::from_seconds(0.1, TimerMode::Once),
        ...
        let mut sched = Schedule::default();
        sched.add_systems(tick_portal_suck);
        for _ in 0..4 {
            sched.run(&mut world);
        }
        let run = world.resource::<Run>();
        assert_eq!(run.floor, 2);
        assert!(!run.portal_open);
        assert_eq!(world.resource::<Toast>().text, "FLOOR 1-2");
        let mut pq = world.query_filtered::<&Pos, With<Player>>();
        assert_eq!(
            pq.iter(&world).next().unwrap().0,
            glam::Vec2::new(10000.0, 10000.0)
        );
        // Level entities (incl. the portal) are gone.
        assert!(world.get_entity(portal).is_err());
    }

    #[test]
    fn floor_transition_completes_run_side_effects() {
        let mut world = portal_world();
        world.insert_resource(FloorTransition {
            active: true,
            stage: 1,
            timer: GTimer::from_seconds(0.05, TimerMode::Repeating),
            progress: 0.0,
        ...
        world.resource_mut::<Run>().gen_seed = 42;
        let player = spawn_player_full(&mut world, Player::default(), 5, 8);
        let mut sched = Schedule::default();
        sched.add_systems(tick_floor_transition);
        for _ in 0..200 {
            sched.run(&mut world);
            if !world.resource::<FloorTransition>().active {
                break;
            }
        }
        assert!(!world.resource::<FloorTransition>().active);
        assert!(!world.resource::<FloorMask>().cells.is_empty());
        // Stage 2 spawns the full level, not just the mask: walls,
        // enemies, and chests exist on the new floor.
        assert!(
            world
                .query_filtered::<(), With<crate::comps_a::WallTile>>()
                .iter(&world)
                .next()
                .is_some(),
            "walls spawn on floor 2"
        );
        assert!(
            world
                ...
                .next()
                .is_some(),
            "enemies spawn on floor 2"
        );
        assert!(
            world
                .query_filtered::<(), With<Pickup>>()
                .iter(&world)
                .next()
                .is_some(),
            "pickups/chests spawn on floor 2"
        );
        let mut hq = world.query_filtered::<&Health, With<Player>>();
        assert_eq!(hq.iter(&world).next().unwrap().hp, 6);
        let started = world.resource_mut::<Queue<FloorStarted>>().drain();
        assert_eq!(started.len(), 1);
        let _ = player;
    }

    // -- unlocks / saves / lifecycle ----------------------------------------

    #[test]
    fn flush_dirty_save_once_single_fires() {
        let mut world = World::new();
        world.insert_resource(SaveDirty(true));
        let mut sched = Schedule::default();
        sched.add_systems(flush_dirty_save_once);
        sched.run(&mut world);
    ...
    #[test]
    fn flush_dirty_save_clears_after_five_seconds() {
        let mut world = World::new();
        world.insert_resource(sim_time());
        world.insert_resource(SaveDirty(true));
        let mut sched = Schedule::default();
        sched.add_systems(flush_dirty_save);
        for _ in 0..100 {
            sched.run(&mut world);
        }
        assert!(world.resource::<SaveDirty>().0);
        // 5 s threshold (+ float margin).
        for _ in 0..100 {
            sched.run(&mut world);
        }
        assert!(!world.resource::<SaveDirty>().0);
    ...
    #[test]
    fn floor_reach_unlocks_crystal_and_dirties() {
        let mut world = World::new();
        ...
        world.init_resource::<SaveData>();
        let mut sched = Schedule::default();
        sched.add_systems(apply_floor_reach_unlocks);
        sched.run(&mut world);
        assert!(world.resource::<SaveDirty>().0);
        assert!(
            world
        ...
        // Second run at the same floor: no repeat.
        world.resource_mut::<Toast>().text.clear();
        sched.run(&mut world);
        assert!(world.resource::<Toast>().text.is_empty());
    }

    #[test]
    fn boss_info_reports_first_boss() {
        let mut world = World::new();
        world.spawn((
            Enemy {
                kind: crate::data::EnemyKind::Bandit,
        ...
        let mut sched = Schedule::default();
        sched.add_systems(|q: Query<(&Enemy, &Health), With<Enemy>>| {
            assert_eq!(boss_info(&q), Some((30, 40)));
        });
        sched.run(&mut world);
    }

    #[test]
    fn cleanup_run_despawns_and_resets_mask() {
        let mut world = World::new();
        world.insert_resource(FloorMask::default());
        world.resource_mut::<FloorMask>().cells.insert((0, 0));
        world.spawn((GameCleanup, Pos(glam::Vec2::ZERO)));
        world.spawn(Pos(glam::Vec2::ZERO));
        let mut sched = Schedule::default();
        sched.add_systems(cleanup_run);
        sched.run(&mut world);
        assert_eq!(world.query::<&GameCleanup>().iter(&world).count(), 0);
        assert_eq!(world.query::<&Pos>().iter(&world).count(), 1);
        assert!(world.resource::<FloorMask>().cells.is_empty());
    }

    #[test]
    fn teardown_game_drains_floaters_and_particles() {
        let mut world = World::new();
        world.insert_resource(FloorMask::default());
        world.resource_mut::<FloorMask>().cells.insert((0, 0));
        let mut seed = Schedule::default();
        seed.add_systems(|mut commands: Commands| {
            commands.spawn(GameCleanup);
            repame_fx::spawn_number(&mut commands, 0.0, 0.0, "5", [1.0, 1.0, 1.0, 1.0]);
            let mut rng = rand::rng();
        ...
        seed.run(&mut world);
        world.flush();
        assert_eq!(world.query::<&GameCleanup>().iter(&world).count(), 1);
        assert_eq!(
            world
        ...
        assert_eq!(
            world.query::<&repame_fx::Particle>().iter(&world).count(),
            4
        );
        let mut sched = Schedule::default();
        sched.add_systems(teardown_game);
        sched.run(&mut world);
        world.flush();
        assert_eq!(world.query::<&GameCleanup>().iter(&world).count(), 0);
        assert_eq!(
    ...
    #[test]
    fn loop_transition_jumps_to_floor_16() {
        let mut run = Run::default();
        run.floor = 15;
        let mut lt = LoopTransition::default();
        lt.throne_ii_spawned();
        lt.throne_ii_defeated();
        let mut trauma = Trauma::default();
        assert!(try_apply_loop_portal_transition(&mut run, &mut lt, &mut trauma));
        assert_eq!((run.loop_count, run.floor), (1, 16));
        assert_eq!(run.area, AreaId::Desert);
    }

    #[test]
    fn secret_transition_enters_queued_vault() {
        use crate::secrets::SecretTriggers;
        let mut run = Run::default();
        let mut triggers = SecretTriggers::default();
        triggers.queue(SecretTarget::Vault);
        let got = apply_secret_transition(&mut run, &mut triggers);
        assert_eq!(got, Some(SecretTarget::Vault));
        assert_eq!(run.area, AreaId::Vault);
        assert_eq!(triggers.vaults_entered, 1);
    }
}
