...
mod source_tests {
    use super::*;
    use crate::comps_a::{HitId, LastDamageTaken};
...
#[cfg(test)]
mod revive_couch_tests {
    use super::*;

    #[test]
    fn revive_grace_arms_hurt_pulse_then_fires() {
        // GML Revive/Create_0: alarm[4] = 300, alarm[5] unset.
        let mut r = Revive::new();
        assert_eq!((r.alarm4, r.alarm5), (300.0, 0.0));
        // Generation holds the grace at 300 (Step_0 GenCont/LevCont arm).
        assert!(!revive_step(&mut r, 10.0, true));
        assert_eq!(r.alarm4, 300.0);
        // 300 grace steps arm alarm[5] = 30 (Alarm_4 performs Alarm_5).
        for _ in 0..300 {
            assert!(!revive_step(&mut r, 1.0, false));
        }
        assert_eq!((r.alarm4, r.alarm5), (0.0, 30.0));
        // 30 more steps fire one pulse and re-arm (Alarm_5 self-sets 30).
        for _ in 0..29 {
            assert!(!revive_step(&mut r, 1.0, false));
        }
        assert!(revive_step(&mut r, 1.0, false));
        assert_eq!(r.alarm5, 30.0);
    }

    #[test]
    fn yv_couch_idle_loops_and_airhorn_returns() {
        // GML YungVenuzCouch/Step_0: image_speed = timescale * 0.4
        // (fps-normalized: 0.4 * 12/30 frames per timescale step here).
        let mut c = YvCouch::idle();
        assert_eq!(c.sprite_path(), "images/sprYVBossGamingIdle.png");
        yv_couch_step(&mut c, 1.0, 12.0, 24);
        assert!((c.frame - 0.16).abs() < 1e-6, "got {}", c.frame);
        // Idle wraps past the last frame (GML image_index wrap).
        yv_couch_step(&mut c, 1000.0, 12.0, 24);
        assert!(!c.airhorn && c.frame < 24.0, "got {}", c.frame);
        // Airhorn one-shot plays once then returns to idle frame 0.
        c.play_airhorn();
        assert_eq!(c.sprite_path(), "images/sprYVBossGamingAirhorn.png");
        yv_couch_step(&mut c, 1000.0, 6.0, 22);
        assert!(!c.airhorn, "airhorn must end");
        assert_eq!(c.frame, 0.0);
        assert_eq!(c.sprite_path(), "images/sprYVBossGamingIdle.png");
        // Paused timescale (0 steps) freezes the frame.
        let f = c.frame;
        yv_couch_step(&mut c, 0.0, 12.0, 24);
        assert_eq!(c.frame, f);
    }
}
