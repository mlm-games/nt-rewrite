...
mod tests {
    ...
    #[test]
    fn angle_inc_matches_bevy_law() {
        // Normal/Idpd/Venuz: deterministic 8 + sin(angle/300°).
        for angle in [0.0, 45.0, 137.5, 1000.0, 200000.0] {
            let want = 8.0 + (angle / 300.0f32).to_radians().sin();
            for kind in [SpiralKind::Normal, SpiralKind::Idpd, SpiralKind::Venuz] {
                assert!(
                    (spiral_angle_inc(angle, kind) - want).abs() < 1e-6,
                    "kind {kind:?} angle {angle}"
                );
            }
        }
        // Proto: 10 + 2*sin(angle/300°) ± 1 jitter.
        for angle in [0.0, 90.0, 1000.0] {
            let base = 10.0 + (angle / 300.0f32).to_radians().sin() * 2.0;
            for _ in 0..50 {
                let inc = spiral_angle_inc(angle, SpiralKind::Proto);
                assert!(
                    (inc - base).abs() <= 1.0 + 1e-6,
                    "proto inc {inc} outside ±1 of {base}"
                );
            }
        }
    }
    ...
    fn spiral_kind_follows_gml_areas() {
        ...
        assert_eq!(SpiralKind::for_gml_area(101), Normal);
        assert_eq!(SpiralKind::for_gml_area(102), Normal);
        assert_eq!(SpiralKind::for_gml_area(104), Normal);
        assert_eq!(SpiralKind::for_gml_area(105), Normal);
    ...
    #[test]
    fn target_areas_map_to_gml_ints() {
        let cases = [
            (AreaId::Campfire, 0),
            (AreaId::Desert, 1),
            (AreaId::Sewers, 2),
        ...
        for (area, want) in cases {
            assert_eq!(gml_area_for_area(area), want, "{area:?}");
        }
    }

    #[test]
    fn kind_selection_per_area_loop() {
        assert_eq!(kind_for_area(AreaId::Desert), SpiralKind::Normal);
        assert_eq!(kind_for_area(AreaId::Palace), SpiralKind::Normal);
        assert_eq!(kind_for_area(AreaId::Loop), SpiralKind::Normal);
        assert_eq!(kind_for_area(AreaId::Vault), SpiralKind::Proto);
        assert_eq!(kind_for_area(AreaId::CrownVault), SpiralKind::Proto);
        assert_eq!(kind_for_area(AreaId::HQ), SpiralKind::Idpd);
        assert_eq!(kind_for_area(AreaId::City), SpiralKind::Venuz);
        assert_eq!(kind_for_area(AreaId::Jungle), SpiralKind::Normal);
        assert_eq!(
            SpiralCtl::warmed_up_for_area(AreaId::Jungle).kindpacked(),
            4.0,
            "jungle crystal flag rides kindpacked, not kind"
        );
    }

    #[test]
    fn snapshot_tracks_ctl() {
        let mut ctl = SpiralCtl::warmed_up_for_area(AreaId::Vault);
        for _ in 0..10 {
            ctl.tick_once();
        }
        let snap = ctl.snapshot(1.0);
        assert_eq!(snap.wisps, ctl.ring.as_slice());
        assert_eq!(snap.debris, ctl.debris_ring.as_slice());
        let want_streams: Vec<[f32; 2]> = ctl
            .streams
            .iter()
            .map(|s| [s.lanim, s.langle])
            .collect();
        assert_eq!(snap.streams, want_streams.as_slice());
        assert_eq!(snap.ticks, ctl.ticks);
        assert_eq!(snap.drain_bias, ctl.drain_bias);
        assert_eq!(snap.bg_rgb, [0.0, 0.0, 0.0]);
        assert_eq!(snap.bg_alpha, 1.0);
        assert_eq!(snap.thresh, 2.5);
        assert_eq!(snap.kindpacked, SpiralKind::Proto as u8 as f32);
        assert_eq!(snap.view, VORTEX_VIEW);
        ...
        assert_eq!(snap.ticks, ctl.ticks);
        assert_eq!(snap.drain_bias, 4.0 * 5.5);
        assert_eq!(snap.thresh, 3.0);
        assert_eq!(snap.wisps, ctl.ring.as_slice());
    }

    #[test]
    fn tick_spiral_system_advances_via_simtime() {
        let mut world = World::new();
        let mut time = SimTime::default();
        time.delta_secs = 1.0 / 30.0;
        world.insert_resource(time);
        world.insert_resource(SpiralCtl::warmed_up());
        let t0 = world.resource::<SpiralCtl>().ticks;
        let mut schedule = bevy_ecs::schedule::Schedule::default();
        schedule.add_systems(tick_spiral);
        schedule.run(&mut world);
        assert!((world.resource::<SpiralCtl>().ticks - t0 - 1.0).abs() < 1e-3);
        // No ctl: system is a no-op, not a panic.
        world.remove_resource::<SpiralCtl>();
        schedule.run(&mut world);
    }
    ...
    #[test]
    fn wisp_streams_follow_gml_head_start_and_rate() {
        // GML Spiral/Create_0: lanim = -random(300) in [-300, 0),
        // langle = random_angle in [0, 2pi).
        let ctl = SpiralCtl::warmed_up_for_area(AreaId::Desert).with_seed(1234);
        let live: Vec<(usize, WispStream)> = ctl
            .ring
            .iter()
            .enumerate()
            .filter(|(_, s)| s[2] >= 0.0)
            .map(|(i, _)| (i, ctl.streams[i]))
            .collect();
        assert!(!live.is_empty());
        for (_, s) in &live {
            // Head-start in (-300, 0] plus at most 150 warmup
            // increments of at most 0.5 each.
            assert!(
                s.lanim > -300.0 - 1e-3 && s.lanim <= 150.0 * 0.5 + 1e-3,
                "head-start out of range: {}",
                s.lanim
            );
            assert!(
                s.langle >= 0.0 && s.langle <= std::f32::consts::TAU,
                "langle out of range: {}",
                s.langle
            );
        }
        // GML Spiral/Step_0: lanim += 0.2 + random(0.3) per tick.
        let mut ctl = SpiralCtl::warmed_up_for_area(AreaId::Desert).with_seed(7);
        for _ in 0..10 {
            // Track the youngest wisp: the next birth lands on a
            // different slot, so it cannot be reborn mid-check.
            let youngest = ctl
                .ring
                .iter()
                .enumerate()
                ...
                .map(|(i, _)| i)
                .expect("warmup births wisps");
            let before = ctl.streams[youngest].lanim;
            ctl.tick_once();
            let d = ctl.streams[youngest].lanim - before;
            assert!(
                d >= 0.2 - 1e-4 && d <= 0.5 + 1e-4,
                "rate out of [0.2, 0.5]: {d}"
            );
        }
    }

    #[test]
    fn wisp_streams_are_seed_stable_and_seed_sensitive() {
        let a = SpiralCtl::warmed_up_for_area(AreaId::Desert).with_seed(99);
        let b = SpiralCtl::warmed_up_for_area(AreaId::Desert).with_seed(99);
        assert_eq!(a.streams, b.streams, "same seed must snapshot identically");
        assert_eq!(a.snapshot(1.0).streams, b.snapshot(1.0).streams);
        let c = SpiralCtl::warmed_up_for_area(AreaId::Desert).with_seed(100);
        assert_ne!(
            a.streams, c.streams,
            "different seeds must roll different streams"
        );
        // Slot correspondence: every live ring slot carries a live
        // stream, every dead ring slot a dead one.
        for (slot, w) in a.ring.iter().enumerate() {
            if w[2] >= 0.0 {
                assert!(
                    a.streams[slot].lanim > -300.0 - 1e-3,
                    "live wisp needs a live stream"
                );
            } else {
                assert_eq!(a.streams[slot].lanim, -1.0);
            }
        }
    }

    #[test]
    fn venuz_starfield_carries_no_bolt_streams() {
        let ctl = SpiralCtl::warmed_up_for_gml_area(103).with_seed(5);
        assert!(ctl.streams.iter().all(|s| !s.bolt_visible()));
        assert!(ctl.streams.iter().all(|s| s.lanim == -1.0));
    }

    #[test]
    fn kill_and_done_follow_despawn_gate() {
        let mut ctl = SpiralCtl::warmed_up();
        assert!(!ctl.is_done());
        ctl.kill();
        assert!(ctl.death_tick.is_some());
        assert!(!ctl.is_done());
        for _ in 0..26 {
            ctl.tick_once();
        }
        assert!(ctl.is_done());
        // Second kill is a no-op: death_tick stays put.
        let death = ctl.death_tick;
        ctl.kill();
        assert_eq!(ctl.death_tick, death);
    }
}
