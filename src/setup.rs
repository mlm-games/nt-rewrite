...
mod tests {
    use super::*;
    use crate::worldgen::generate_level;

    fn run_for(floor: u32) -> crate::comps_a::Run {
        let loop_count = (floor.max(1) - 1) / 15;
        crate::comps_a::Run {
            floor,
            world: crate::worldgen::world_of(floor),
            area: crate::data::AreaId::from_route_floor(floor),
            loop_count,
            floor_in_area: crate::worldgen::floor_in_world(floor),
    ...
    #[test]
    fn mask_covers_plan_cells() {
        let run = run_for(1);
        let plan = generate_level(&run);
        assert!(!plan.floor_cells.is_empty());
        let mask = build_floor_mask(&plan);
        assert_eq!(mask.cells.len(), plan.floor_cells.len());
        assert_eq!((mask.cols, mask.rows), (80, 52));
        let (cx, cy) = plan.floor_cells[0];
        assert!(mask.is_walkable(mask.cell_center((cx, cy))));
    }

    fn empty_catalog() -> repame_anim::AnimCatalog {
        repame_anim::AnimCatalog::from_json(
            "{}",
            repame_anim::AtlasDesc {
                size: 128,
    ...
    #[test]
    fn decal_kinds_spawn_pulse_records_verbatim() {
        use crate::environment::{PulseSprite, SurfacePulse, SurfaceZone};

        let run = run_for(1);
        let catalog = empty_catalog();

        // Cobweb: zone + subtle pulse + 36px decal (no art in the empty
        // catalog -> solid-fallback record).
        let mut world = bevy_ecs::prelude::World::new();
        let pos = glam::Vec2::new(100.0, 0.0);
        let mut cmds = world.commands();
        let out = spawn_prop_sim(&mut cmds, &catalog, &run, PropKind::Cobweb, pos);
        world.flush();
        assert!(out.is_none());
        let (zone, pulse, vis) = world
            .query::<(&SurfaceZone, &SurfacePulse, &PulseSprite)>()
            .single(&world)
            .unwrap();
        ...
        // FireTrap: hazard entity with the trap phase law + 32px visual.
        let mut world = bevy_ecs::prelude::World::new();
        let pos = glam::Vec2::new(100.0, 50.0);
        let mut cmds = world.commands();
        let out = spawn_prop_sim(&mut cmds, &catalog, &run, PropKind::FireTrap, pos);
        world.flush();
        assert!(out.is_none());
        let (hazard, pulse, vis) = world
            .query::<(
                &crate::environment::EnvironmentHazard,
                &SurfacePulse,
                &PulseSprite,
            )>()
            .single(&world)
            .unwrap();
        assert_eq!(hazard.spec.radius, 42.0);
        assert!((pulse.phase - 1.1).abs() < 1e-4, "got {}", pulse.phase);
        assert_eq!(vis.size, 32.0);
    ...
    #[test]
    fn setup_run_builds_fish_defaults_with_revolver_and_ammo() {
        let mut world = bevy_ecs::prelude::World::new();
        ...
        assert!(!world.resource::<DeferredFloorGen>().0);

        let run = world.resource::<Run>();
        assert_eq!(run.floor, 1);
        assert_eq!(run.world, 1);
        assert_eq!(run.area, crate::data::AreaId::Desert);
        assert_eq!(run.loop_count, 0);
        assert_eq!(run.gen_seed, 0x1234_5678);
        assert!(!run.portal_open);

        assert!(!world.resource::<FloorMask>().cells.is_empty());

        let started = world.resource::<Queue<FloorStarted>>();
        assert_eq!(started.len(), 1);

        let mut q = world.query::<(&RaceState, &Inventory, &Health, &Player)>();
        let (rs, inv, health, player) = q.single(&world).unwrap();
        assert_eq!(rs.race, RaceId::Fish);
        assert_eq!(rs.skin, SkinLetter::A);
        assert_eq!(inv.weapons[0], WeaponId::REVOLVER);
        assert_eq!(inv.weapons[1], WeaponId::NONE);
        assert_eq!(inv.weapon_slots, 2);
        // Fish bullets: (32 + 8) * 3.
        assert_eq!(inv.ammo[1], 120);
        assert_eq!((health.hp, health.max), (8, 8));
        assert_eq!(player.crown, CrownKind::None);
        assert_eq!(player.pickup_range, 95.0);
        // No crown: no toast.
    ...
    #[test]
    fn setup_run_gates_preferred_skin() {
        let mut world = bevy_ecs::prelude::World::new();
        world.init_resource::<SaveData>();
        world
            ...
            .preferred_skin = 2;
        setup_run_with_seed(&mut world, 99);
        let mut q = world.query::<&RaceState>();
        assert_eq!(q.single(&world).unwrap().skin, SkinLetter::A);
        ...
        world.init_resource::<SaveData>();
        {
            ...
            lo.preferred_skin = 2;
            lo.unlocked_skins[2] = true;
        }
        setup_run_with_seed(&mut world, 99);
        let mut q = world.query::<&RaceState>();
        assert_eq!(q.single(&world).unwrap().skin, SkinLetter::C);
    }

    #[test]
    fn setup_run_applies_destiny_crown_loadout() {
        let mut world = bevy_ecs::prelude::World::new();
        world.init_resource::<SaveData>();
        world
            .resource_mut::<SaveData>()
            .unlock_crown(RaceId::Fish, crate::savedata_part::crown_port_to_gml(7));
        setup_run_with_seed(&mut world, 7);

        let mut q = world.query::<(&Player, &Inventory)>();
        let (player, inv) = q.single(&world).unwrap();
        assert_eq!(player.crown, CrownKind::Destiny);
    ...
    #[test]
    fn sanitize_clamps_oob_weapon_ids() {
        assert_eq!(sanitize_weapon_id(WeaponId(255)), WeaponId::NONE);
        assert_eq!(
            sanitize_weapon_id(WeaponId(crate::weapons_data::WEAPON_COUNT as u8)),
            WeaponId::NONE
        );
        assert_eq!(sanitize_weapon_id(WeaponId(5)), WeaponId(5));
        assert_eq!(resolve_start_weapon(WeaponId::NONE), WEAPON_REVOLVER);
        // Corrupt weapons grant no ammo.
        assert_eq!(
            starting_ammo_for(
    ...
    #[test]
    fn starting_ammo_for_per_ammo_kind() {
        // Revolver: Robot 32*3, Fish (32+8)*3.
        assert_eq!(
            starting_ammo_for(
        ...
        // Shotgun shells, crossbow bolts, grenade explosives, laser energy.
        let shells = starting_ammo_for(
            &[WeaponId::SHOTGUN, WeaponId::NONE, WeaponId::NONE],
            RaceId::Robot,
            CrownKind::None,
        );
        assert_eq!(shells[2], 24);
        let bolts = starting_ammo_for(
            &[WeaponId::CROSSBOW, WeaponId::NONE, WeaponId::NONE],
            RaceId::Robot,
            CrownKind::None,
        );
        assert_eq!(bolts[3], 21);
        let boom = starting_ammo_for(
            &[WeaponId::GRENADE_LAUNCHER, WeaponId::NONE, WeaponId::NONE],
            RaceId::Robot,
            CrownKind::None,
        );
        assert_eq!(boom[4], 18);
        let energy = starting_ammo_for(
            &[WeaponId(19), WeaponId::NONE, WeaponId::NONE],
            RaceId::Robot,
            CrownKind::None,
        );
        assert_eq!(energy[5], 30);
        // Haste +1 per kind before x3; Fish+Haste shotgun shells.
        let hasted = starting_ammo_for(
            &[WeaponId::REVOLVER, WeaponId::NONE, WeaponId::NONE],
            RaceId::Robot,
            CrownKind::Haste,
        );
        assert_eq!(hasted[1], 99);
        let fish_haste = starting_ammo_for(
            &[WeaponId::SHOTGUN, WeaponId::NONE, WeaponId::NONE],
            RaceId::Fish,
            CrownKind::Haste,
        );
        assert_eq!(fish_haste[2], (8 + 2 + 1) * 3);
        // Melee grants nothing.
        assert_eq!(
            starting_ammo_for(
                &[WeaponId::WRENCH, WeaponId::NONE, WeaponId::NONE],
                RaceId::Fish,
                CrownKind::None
            ),
            [0; MAX_AMMO_TYPES]
    ...
    #[test]
    fn deferred_floor_gen_kicks_and_is_consumed() {
        use crate::comps_b::FloorTransition;
        use crate::progression::try_start_pending_floor_gen;

        // Kick: pending floor gen inserts an active transition.
        let mut world = bevy_ecs::prelude::World::new();
        ...
        // Consumed: the mutation-choice flow clears the flag once picks
        // resolve (mirrors the progression-system resource set).
        use crate::audio::{AudioCue, GameAudio};
        use crate::comps_a::MutationChoice;
        use crate::effects::ChromaticAberration;
        use repame_fx::Trauma;
        let mut world = bevy_ecs::prelude::World::new();
        world.init_resource::<MutationChoice>();
        world.init_resource::<crate::state::Paused>();
        world.insert_resource(DeferredFloorGen(true));
        world.init_resource::<ScarierFace>();
        world.init_resource::<Euphoria>();
        world.init_resource::<OpenMind>();
        world.init_resource::<HeavyHeart>();
        ...
        world.insert_resource(crate::comps_a::PendingMutation {
            choices: vec![crate::data::MutationId::EagleEyes],
        });
    ...
    #[test]
    fn spawn_level_fields_floor_from_plan() {
        let mut world = bevy_ecs::prelude::World::new();
        setup_run_with_seed(&mut world, 0xBEEF);
        let plan = {
            let run = world.resource::<Run>();
            worldgen::generate_level(&run)
        };
        ...
        // Mask rebuilt from the same plan.
        assert_eq!(
            world.resource::<FloorMask>().cells.len(),
            plan.floor_cells.len()
        );
        // Wall bodies merged from wall + small-wall cells.
        let mut expected_walls = plan.wall_cells.clone();
        for &(wx, wy) in &plan.small_walls {
        ...
        let sim_props = plan
            ...
            .filter(|(k, _)| {
                !matches!(
                    k,
                    PropKind::Cobweb | PropKind::IcePatch | PropKind::FireTrap
                )
            })
            .count();
    ...
    #[test]
    fn spawns_carry_table_stats() {
        let mut world = bevy_ecs::prelude::World::new();
        let mut schedule = bevy_ecs::schedule::Schedule::default();
        schedule.add_systems(|mut commands: Commands| {
            spawn_enemy(&mut commands, EnemyKind::Bandit, glam::Vec2::ZERO);
            spawn_player(&mut commands, glam::Vec2::new(10.0, 0.0));
        });
        schedule.run(&mut world);
        let bandit_hp = enemy_def(EnemyKind::Bandit).hp;
        let mut eq = world.query::<(Entity, &Health, &Team)>();
        let mut found_bandit = false;
        let mut found_player = false;
        for (e, h, t) in eq.iter(&world) {
            if *t == Team::Enemy {
                assert_eq!((h.hp, h.max), (bandit_hp, bandit_hp));
        ...
        assert!(found_bandit && found_player);
    }
}
